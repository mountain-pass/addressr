...
import StateAutocomplete from './StateAutocomplete.vue';
...
describe('StateAutocomplete (Vue)', () => {
  ...
  it('emits select with StateSearchResult when option chosen', async () => {
    const onSelect = vi.fn();
    const mockFetch = vi.fn()
      .mockResolvedValueOnce(rootResponse())
      .mockImplementation(() => Promise.resolve(searchResponse()));

    render(StateAutocomplete, {
