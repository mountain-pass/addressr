...
<script lang="ts">
  ...
  export let label: string = 'Search Australian suburbs and towns';
  export let placeholder: string = 'Start typing a suburb or town...';
  export let debounceMs: number | undefined = undefined;
  export let name: string = 'locality';
  export let required: boolean = false;
  export let onSelect: ((result: LocalitySearchResult) => void) | undefined = undefined;
  ...
  const store = createLocalitySearch({ apiKey, apiUrl, apiHost, debounceMs, fetchImpl });

  let isOpen = false;
  let highlightedIndex = -1;

  let query = '';
  let results: LocalitySearchResult[] = [];
  let isLoading = false;
  ...
  function selectItem(index: number) {
    ...
    if (item) {
      ...
      store.setQuery(`${item.name} ${item.state.abbreviation} ${item.postcode}`);
      isOpen = false;
      highlightedIndex = -1;
    }
  }
...
<div class="addressr-wrapper">
  ...
  >
    {#if showMenu}
      ...
      {/if}
      {#each results as item, index (item.pid)}
        <li
          id="{uid}-option-{index}"
          role="option"
          tabindex="-1"
