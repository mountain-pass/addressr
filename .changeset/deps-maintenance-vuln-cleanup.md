...
Removed three unused dependencies that rooted every remaining audit finding: the dead `istanbul-middleware` and `istanbul` dev tools (coverage runs on `nyc`), and the production `js-yaml` dependency orphaned by the v1 Swagger removal. The abandoned `npm-check` dev tool is no longer installed. Its convenience scripts now run it on demand via `npx`. Together these drop the install tree by ~300 packages and bring `npm audit` to zero (previously 1 critical / 8 high / 9 moderate / 2 low, all dev-only).
