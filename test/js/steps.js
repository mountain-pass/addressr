...
Then(
  'the response will contain the following links:',
...
Then(
  'the response will contain the following link template:',
  async function (dataTable) {
    const hashes = dataTable.hashes();
    logger('hashes', hashes);
...
When(
  'the {string} link is followed for {string}',
  async function (relationship, type) {
...
When(
  'the {int}st {string} link is followed',
  async function (nth, relationship) {
    this.prev = this.current;
    this.current = await this.current.ops
      .filter(relationship)
      [nth - 1].invoke();
  },
);
...
Then(
  'the address details will map to the following address:',
  async function (documentString) {
    const expected = JSON.parse(documentString);
    ...
    this.context.localityIndexed[this.addressDetails.LOCALITY_PID] =
      this.locality;
    expect(
...
Then(
  ...
  async function () {
    ...
    for (const a of this.current.json) {
...
Then(
  'the {string} link templates var-base will contain',
  async function (relationship, expectedParameters) {
    this.prev = this.current;
    expect(this.current.linkTemplate).to.not.be.undefined;
...
When(
  'the {string} link template is followed with:',
  async function (relationship, parameters) {
    this.prev = this.current;
    if (this.current.linkTemplate) {
      expect(this.current.linkTemplate).to.not.be.undefined;
      const link = this.current.linkTemplate.get('rel', relationship);
...
Then(
  'the returned response will have a {int} status code',
  ...
  async function (relationship) {
    try {
      ...
      if (this.current.ops) {
        ...
        const item = await this.current.ops.find('item').invoke();
        console.log({ item });
...
Then(
  'the reponse will have a {string} of {string}',
  async function (headerName, headerValue) {
...
Then(
  'the reponse will not have a {string} header',
