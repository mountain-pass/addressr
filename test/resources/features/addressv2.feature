...
Feature: Addresses v2
    ...
    Scenario: P015 Mid-range number returns non-range doc first, range doc absent from list (ADR 028 endpoint-only)
        ...
        # number. Asserts BOTH that the range doc (GAOT_717321171) is absent
        # from the entire result list (v2-capable NOT-include step per P029)
        # AND that the first result is the non-range `106 GAZE RD` record.
        When the root api is requested
        And the "https://addressr.io/rels/address-search" link template is followed with:
    ...
    Scenario: Joint ADR 027 + ADR 028 integration — endpoint recall AND fuzzy exclusion
        ...
        # Restored to the original v2.4.0 dual-assertion form once the
        # NOT-include step became v2-capable (P029).
        When the root api is requested
        And the "https://addressr.io/rels/address-search" link template is followed with:
            | q | 107 GAZE RD, CHRISTMAS ISLAND |
        Then the returned address list will include:
