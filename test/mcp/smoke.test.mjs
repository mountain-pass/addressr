...
describe(
  'MCP Smoke Tests',
  { skip: !hasRapidApiKey() && 'RAPIDAPI_KEY not set' },
  () => {
    let client;
    ...
    it('searches for addresses', async () => {
      ...
      const results = Array.isArray(parsed)
        ? parsed
        : parsed.results || parsed.addresses || [parsed];
    ...
    it('retrieves address details', async () => {
      ...
      assert.ok(
        results.length > 0,
        'Need search results to test detail retrieval',
      );
      ...
      assert.ok(
        pid,
        `Expected pid in search result. Keys: ${Object.keys(firstResult).join(', ')}`,
      );
      ...
      const detailText = detailResult.content.find(
        (c) => c.type === 'text',
      )?.text;
