...
| P025 | GitHub Actions using Node.js 20 runtime are deprecated             | 2026-07-19 | no — not observed | Five Node 20/16-era pins bumped to Node 24 builds (cache@v6 ×2, changesets/action@v1.9.0, checkout@v5, setup-node@v5, aws-credentials@v6) per RFC-005 (workflow-only, no changeset); verify zero Node deprecation annotations on next release.yml + reusable-update.yml runs |
...
**Closed (2026-07-19)** — P027 (synonym expansion bypasses AUTO:5,8 fuzziness): relevance-close, no fix released. Defect not reproducible on the current prod stack (OpenSearch 3.5, sole domain since the 2026-07-14 ADR-035 cutover) — full 8-query ticket battery clean via the RapidAPI gateway. Original hypothesis falsified: the synonym rules are one-way contractions (`ROAD => RD`), so `Rd` and `ROAD` query forms analyze to identical token streams and synonym expansion cannot explain the April 1.3.20 observation; a minimal 1.3.20 repro with the exact prod config also did not reproduce. The 1.3.20 domain is decommissioned; the April mechanism is unknowable (no `explain=true` was captured). Reopen path documented in the ticket. Same platform-migration basis as the P028 close. The battery doubles as observed verification evidence for P026 (see Verification Queue).
