...
| ---- | ---- | ----------------------------------------------------- | ---------- | ----------- | ------ | ------------------------------------------------------------------- |
| 16.0 | P005 | TDD hook does not recognise Cucumber feature files    | 8 (Medium) | Known Error | S      | Fix in windyroad TDD plugin (external repo) — limited actionability |
...
| 6.0  | P019 | No deploy-time smoke check for root Link header rels  | 6 (Medium) | Open        | S      | Add curl+grep probe to `.github/workflows/release.yml`              |
