...
- [x] **Denominator computed 2026-08-02** from the primary's representative pre-cutover traffic — whole UTC days only, with the ramp day excluded by a median-relative filter, leaving 20 representative days. The gate was met at **157%** (0.392× served against the 0.25× threshold). Absolute counts are confidential under `RISK-POLICY.md` and this repo is public, so only the ratio is recorded.
- [x] **Encoded at both read-sites 2026-08-02**: `docs/OPENSEARCH-MIGRATION-PLAYBOOK.md` at the decommission step, and the `deploy/main.tf` comment on the standby module. The playbook site is the load-bearing one — the `main.tf` module block was destroyed with the standby, so the playbook is what governs the NEXT migration.
- [x] **R010 curated 2026-08-03** (commit `9507d29`): treatment set to the retention condition, re-read trigger corrected from "any new pipeline hint with this risk_slug" to "a standby decommission is proposed", and both states of the world scored so the ADR-035 Option C trade is measured (5/25) rather than assumed.

  **The routing instruction above was unsatisfiable and the entry was hand-written instead.** `/wr-risk-scorer:create-risk` Step 3 mints a _new_ ID — the skill has no path for editing an existing entry, and the plugin ships no curate or update sibling. Hand-writing against the skill's own Step-5 entry shape is the only available route today. Captured as P083, which also tracks the upstream report.
...
- **Blocks**: decommissioning `addressr5` (v3). Per `deploy/main.tf` and the P069 runbook, v3 was eligible only after the rollback exercise — that happened 2026-08-02, the retention gate was met at 157%, and `addressr5` was destroyed the same day over two staged applies (`96e965c`, `2e557b9`). So this blocks nothing today; it blocks the NEXT cycle, and the remaining open task is whether the condition gets enforced rather than documented.
- **Blocked by**: (none)
- **Composes with**: P069
...
- **R010** (`docs/risks/R010-warm-standby-decommission-removes-instant-rollback-net.active.md`) — the register entry this ticket is the treatment for. It carried no baseline into any of the three cutovers because every scoring field held the auto-scaffold sentinel; **curated 2026-08-03 in commit `9507d29`**, inherent 15 / residual 5, with the re-read trigger fixed to fire at the decision point.
- **P083** — the other 24 register entries still carry that sentinel. R010 is the worked example and P083 carries the triaged backlog, including the upstream report that `create-risk` cannot edit an existing entry.
