...
- [ ] Set the retention threshold as a **multiple of average daily traffic** on the primary since cutover — e.g. "the standby is retained until the primary has served N× its average daily request volume". User direction 2026-08-02, and it is the right shape for three reasons: it is a ratio, so it discloses no absolute request count and is safe to commit to a public repo (absolute request and read counts are confidential traffic volumes under RISK-POLICY); it self-adjusts as traffic grows or shrinks, where a fixed calendar date silently gets stricter or looser; and it measures the thing that actually retires the risk — successful requests served — rather than elapsed time, which retires nothing on a quiet week. Compute N× from `SearchRate` on the primary; commit the multiple and the resulting go/no-go, never the underlying counts.
