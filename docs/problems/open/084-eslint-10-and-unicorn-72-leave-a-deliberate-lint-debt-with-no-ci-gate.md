...
- [ ] Work `unicorn/no-this-outside-of-class` (173). Check whether these are genuine CJS-era `this` usages that should become explicit parameters or module-scope references, or whether the rule is wrong for this codebase and belongs in the permanent-off list — which as of 2026-08-09 has ONE member, `unicorn/prefer-top-level-await`, off for two remaining promise-chain process entry points (`loader.js:40`, `src/server2.js:37`) and NOT for a module-system reason. `unicorn/prefer-module` left that list the same day: ADR-044 retired the blocker and a repo-wide run measured zero violations, so it is now `error`. Note the precedent has changed shape, not just size — the surviving member's argument is about entry-point startup, so a module-system case for `no-this-outside-of-class` no longer has anything to sit next to.
...
- [ ] **Wire `apps/addressr-deployment/cloudflare-worker/worker.test.js` into a runner.** Added 2026-08-10. It is DARK: no glob reaches it — `test:js` is `test/js/__tests__/*.test.mjs`, and `test:precommit`, `test:mcp:smoke` and `test:integration:search` all point elsewhere — so its assertions have never executed on any commit. That matters beyond tidiness: it is the only instrument for ADR-032's confirmation criterion 4, which encodes JTBD-200's partial-configuration-fails-loud requirement (a `RAPIDAPI_KEY`-missing fetch must return 500 with `RAPIDAPI_KEY not configured`) on the gateway boundary of the revenue-earning API. Terraform gates the deploy-time half — the binding is a required module variable, so a missing value fails at plan — but the runtime fail-loud branch is asserted by nothing.

  **Why it is parked here rather than in its own ticket:** the work is the same shape and the same files as the `.mjs` items above, and it lands with the Worker lint pass. **Recorded because a repoint deferred to this ticket and this ticket did not own the task** — the risk scorer caught that on 2026-08-10, which is exactly the deferral-points-at-nothing failure that leaves work unowned. Do not treat this as a formatting item: wiring it up is a behaviour change (a new tier or a widened glob, plus whatever the assertion does under a runner that has never run it), so it may red on first execution. **That red is the point, not a regression.**
