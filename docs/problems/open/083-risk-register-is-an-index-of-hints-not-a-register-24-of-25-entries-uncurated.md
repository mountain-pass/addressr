...
- [x] **R021 and R003 curated 2026-08-04, closing the apply-axis group.** Both verified in source rather than from their descriptions, and the pair demonstrates why the consolidation was rejected.
  - **R021** (15 inherent / **10 residual, above appetite**, Mitigate — re-rated 2026-08-09 from 5/Accept when its Monitoring trigger fired on a failed push-tier apply) — the push-tier trigger. `release.yml` gates three deploy entry points differently, and this is the only one where a routine `git push` reaches production infrastructure, with no plan-approval step and no blue/green. Base rate as at 2026-08-04 is three successful production applies (`33e6c04`, `96e965c`, `2e557b9`); a fourth followed 2026-08-05 against an empty plan, so it exercised the trigger rather than an apply — see R021's Monitoring cell, which is the canonical count. Held AT the appetite line rather than below it deliberately, until an event settled it: the gap between "an apply starts" and "an apply is reviewed" is unclosed by design on this path, three applies was a small sample against a Severe impact, and an entry at 5 kept the gap legible. Scoring it lower on the base rate would have made the register agree with the axis rather than describe it. On 2026-08-08 a push-tier apply failed (run `31252424980`, P095), and the entry's own trigger moved likelihood off Rare: it now reads 10 and above appetite, with the treatment named and awaiting maintainer ratification.
...
**Zero entries carry the ADR-026 ungrounded-output sentinel.** Every one of the 14 active entries has grounded scores (16 before R021 and R022 retired on 2026-08-10), a control list that separates evidenced from procedural, a treatment decision, a named owner, and a re-assess trigger keyed on the hazard rather than on scorer activity.

**Eleven of the 14 sit above appetite** — R006 and R020 at 10, R004 and R027 at 9, R003/R008/R009/R015 at 8, R007, R012 and R028 at 6. Of the remaining three, R010 sits exactly at 5, which is within appetite because the threshold is inclusive.

Updated 2026-08-10, when R021 and R022 retired with the `deploy/**` push axis and R020 re-scored 8 → 10. The partition moved for two different reasons at once and they are worth keeping apart: the denominator fell because two entries left the register, and R020 crossed the line because retiring R021 **deleted the ground its Impact 4 rested on** — its own text held it below Severe _"because the primary path is now proven and would usually be available"_, and that path no longer exists. So the count improving from twelve to eleven is not a reduction in exposure; one entry got materially worse in the same sitting.

Superseded, retained as history: R021 joined the partition on 2026-08-09, moving 5 → 10 when its Monitoring trigger fired; it was the first entry in this register to cross the line on a realised event rather than on a re-reading. R020 briefly read 4 on 2026-08-05 before the scorer withdrew the drop: exercising the path retired its plumbing sub-hazard only, and both exercises ran against a plan that changed nothing — which is why its 2026-08-10 raise lands on a path that still has no real-mutation exercise behind it. R028 moved 8 → 6 in the same sitting when two mechanical checks landed; a first draft put it at 4 and within appetite, and the scorer withdrew that because one check covered a single phrasing and the other did not run in the gate at all.

That is what an honestly-scored register looks like, not a backlog signal. Scoring any of them down to 5 to tidy the number would be the failure this ticket was opened about — and the temptation is real precisely because entries already sit on the line. **De-cardinalised 2026-08-09**, when R021 crossed to 10 and this sentence still said three: the tally is stated correctly two lines above, and restating it here bought nothing except a second place to go stale. Another instance of the class R028 owns — that entry is where the class is catalogued, and it deliberately keeps no running total — and the register-scoped, same-file-different-section shape it predicted — legitimately outside every invariant, since the partition check is scoped to the bolded above-appetite form and this was an unbolded within-appetite tally.
