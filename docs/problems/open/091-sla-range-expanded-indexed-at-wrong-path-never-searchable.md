...
Three layers of green over a field that does not work. This is tracked as its own problem — [P033](../closed/033-source-inspection-tests-anti-pattern.md), which **named this exact example when it was filed on 2026-04-28**, eight days after the defect landed:
...
- [x] **Decided 2026-08-09 — see the DECIDED entry above.** Deprecate first, remove at the next full load. Original framing follows.
...
- [x] **Spec-vs-response join finished 2026-08-09.** The expected key set is now computed by production code — `buildIndexedDocument`'s own `const { sla, ssla, ...structured } = item` over a mapped fixture — so changing the destructure moves the expectation with it. Mutation-proved: altering the production destructure fails it. The fixture is still hand-shaped, so a key added to `mapAddressDetails` that the fixture lacks still slips; closing that needs a fixture built by `mapAddressDetails` itself, which needs a G-NAF row plus auth-code context. That residue is stated at the site rather than implied.
- [x] **Join extended to `api/swagger-2.yaml` 2026-08-09**, against the same derived set, so the two specs cannot drift from each other or from the response. Mutation-proved in both directions — dropping a real property and re-adding the phantom `ssla`.
- [x] **Stale-babel class swept 2026-08-09 — three sites, not the one the task named.** `address-service.test.mjs` claimed the module is babel-only and unimportable (verified false: it imports with 15 exports), `build-indexed-document.test.mjs` claimed the same upstream, and `graceful-shutdown.test.mjs` claimed `src/server2.js` is blocked by the babel-only server module. That last one is half-true and now says so: the babel half is dead, the side-effecting-entry half is the real blocker. Found by bare-token grep, which is what the task said to use and what the phrase grep missed.
- [x] ~~Finish the spec-vs-response join.~~ **Done 2026-08-09 — see the ticked entry above, which supersedes the analysis in this one.** Kept because the mechanism it names is the one the fix used. Original text: the guard added 2026-08-09 (`waycharter-server.test.mjs`) catches drift of the SPEC against a hand-maintained key list — a key dropped, a phantom added, a notice stripped, all mutation-proved. It does NOT catch drift of the CODE against that list, which is the mechanism that produced this ticket: add a key in `mapAddressDetails` and the response grows one while every case stays green. The derivation that closes it is cheap — `buildIndexedDocument` does `const { sla, ssla, ...structured } = item`, so `Object.keys(buildIndexedDocument({ item }).structured)` IS the response key set, computed by production code. Also extend the join to `api/swagger-2.yaml`, which ships in the tarball, carries two of this batch's three corrections, and has no test of any kind.
- [x] ~~**Sweep the CLASS, not the instance: post-ADR-044 "babel-only" and "cannot be imported by raw Node ESM" claims, wherever they live.**~~ **Swept 2026-08-09, and the universe is stated rather than implied: code, workflows, scripts, ADRs, RFCs and JTBDs.** Nine sites corrected — three test-file comments (`address-service.test.mjs`, `build-indexed-document.test.mjs`, `graceful-shutdown.test.mjs`), two `eslint.config.js` rule suppressions whose stated blocker was superseded ADR-005 (`unicorn/prefer-module` measured clean repo-wide and enabled at `error`; `unicorn/prefer-top-level-await` re-reasoned onto its two real entry-point violations), one ADR Confirmation criterion (ADR-033's `babel-node loader.js`, unexecutable as written), two RFCs (RFC-007's `babel-node` workflow commands — a forward instruction for a workflow that does not exist yet, so it would have been transcribed faithfully; and RFC-001's Babel `lib/` build), and JTBD-203's governing-decision citation. P084's own text was corrected too: its ADR-005 bullet instructed a future sweeper to file a new rule "alongside them with the same citation", i.e. to add a FRESH ADR-005 citation for a constraint that had lifted the day before.

  **Rule for the next sweeper, so the surviving hits are not re-litigated.** The bare token `babel` still appears in about ten places and every one is a TRUE PAST-TENSE DATED statement — `src/waycharter-server.js:1072`, `cucumber.js:37`, `.github/workflows/release.yml:66,150`, `utils/stream-down.js:5`, `scripts/push-and-watch.sh:108`, three test files, and both surviving comment sites here. Past-tense-and-dated is fine; present-tense is the defect. `docs/BRIEFING.md.migrated-2026-07-26` and `plan.md` are excluded by rule rather than omitted — a dated archive and a historical migration plan — and ADR-020's Babel lines are MADR Context at decision time.

  **A fourth excluded class, added 2026-08-09 after the maintainer set the amendment-discipline rule**: superseded wording QUOTED inside a ratified, shipped decision's amendment note. `ADR-014`'s Decision Outcome and `ADR-033`'s Confirmation criterion and Decision Outcome now carry the old text verbatim, which puts the token back on this grep surface — deliberately, because a shipped decision's record is the audit trail. The quoted sentence may be present tense inside its quotes; the FRAME is what must be dated and past-tense. So the sweeper's test is not "present tense is the defect" flat: it is "present tense OUTSIDE a dated retention frame is the defect". See `DECISION-MANAGEMENT.md` § Amending a Decision in Place.

  **Superseded text was deleted in CODE COMMENTS and retained in DECISION RECORDS, and the line between them is a lifecycle rule the maintainer set on 2026-08-09**: rewrite-in-place is fine while a decision is still being iterated; once it is ratified AND implemented in production it MUST retain its superseded text as history. A shipped decision's record is the audit trail, and editing it to read as though it always said the right thing removes the evidence that it did not. Working material — test comments, notes on an unimplemented proposal — carries no such duty, and there the grep argument below applies. Two of this sweep's own ADR edits got that backwards and were corrected the same day: ADR-014's Decision Outcome, where the superseded wording was described rather than quoted, and ADR-033's Confirmation criterion, where one word was changed with no note at all.
...
- [ ] Add the test that would have caught this: index a range document and assert what the query can actually find. End-to-end, not a return-value check and not a source-inspection regex — see [P033](../closed/033-source-inspection-tests-anti-pattern.md).
...
- [P033](../closed/033-source-inspection-tests-anti-pattern.md) — the test class that let this stay green for four months, and which named this exact example in its Description eight days after the defect landed. Re-rated on this evidence.
