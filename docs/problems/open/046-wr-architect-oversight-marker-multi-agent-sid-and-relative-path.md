...
### Recurrence evidence — 2026-07-28 (JTBD-202 confirm, RESUMED session + classifier-blocked escape)

Two new dimensions: the **resumed/compacted-session** case, and the **wr-jtbd** shim (`wr-jtbd-mark-oversight-confirmed`) — the sibling of the wr-architect command this ticket names. On a resumed session BOTH candidate-SID sources are empty: `$CLAUDE_SESSION_ID` is unset in the Bash tool env, AND no `/tmp/*-announced-<live-sid>` marker exists (a resume never writes one). The shim's candidate set is therefore ~24 STALE sids from prior sessions; it writes markers under all of them, misses the live sid entirely, prints nothing, and exits 0 — so it looks like it worked. The discipline hook (live sid from stdin = `2f4ccb8f-…`, which equals the newest `~/.claude/projects/<proj>/*.jsonl` basename and the `claude --resume <uuid>` value the CLI prints) finds no matching marker and denies.

New wrinkle that removes the manual escape: the auto-classifier BLOCKS the agent from both `CLAUDE_SESSION_ID=<id> <shim>` and hand-`touch`ing the marker (both read as gate-bypassing). So the P043 / this-ticket "hand-write the marker for each candidate UUID" workaround is unavailable to the agent on a resumed session. Recovery required the USER to run the shim via the `!` prefix with the id set: `! CLAUDE_SESSION_ID=<transcript-uuid> wr-jtbd-mark-oversight-confirmed <path>`, then retry the Edit — a plain user `!` run WITHOUT the env prefix also missed (their shell had empty `CLAUDE_SESSION_ID` too). Confirms the upstream fix must be the shim resolving the live sid the way the hook does (stdin / authoritative runtime-sid file per the P124/P260 pattern), never mtime-spray. Adds `@windyroad/wr-jtbd` as a second upstream target alongside `@windyroad/wr-architect`. Session memory: `reference_resumed_session_oversight_marker_defect.md`.
