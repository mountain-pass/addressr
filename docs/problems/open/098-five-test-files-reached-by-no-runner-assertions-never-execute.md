...
- [ ] **New 2026-08-24 — a guard now reports this tier HEALTHY while nothing runs it.** `scripts/assert-test-files.mjs` was extended to floor every glob-driven tier, `test/precommit/` included, and it prints `2 in test/precommit/ matching *.test.mjs (floor 1)` on every pre-commit. That is true and it is the wrong reassurance: the guard answers "if this tier runs, does its glob match enough files", never "does this tier run". So this ticket's sharpest instance now has a green light in front of it. The third axis the guard does not read is REACHABILITY, and `npm-script-contracts.test.mjs` already implements exactly that check for a different script pattern (`/^test:.+:(?:geo|nogeo)$/`, filed for P094) — extending its predicate to `node --test` tiers is probably cheaper than anything else in this list, and would close the class rather than this instance.
