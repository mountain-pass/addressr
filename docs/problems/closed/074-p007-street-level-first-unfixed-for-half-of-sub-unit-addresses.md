...
Maintainer verified on the live site and accepted. The property evidence behind that acceptance — 0 violations of 150 on a freshly redrawn national sample against a 60.0%-of-120 baseline — is recorded on [P007](007-search-scoring-exact-address-ranked-below-subunits.md)'s closure note, which is the ticket the reporter's issue tracks.

[ADR-043](../../decisions/043-keyword-prefix-anchor-for-street-level-first-ranking.accepted.md) promoted to `accepted` the same day, with Confirmations 4 and 7 discharged.

Two open tasks were re-homed rather than closed with this ticket: the CHANGELOG erratum and the widen-the-sample characterisation, both now on [P092](../open/092-changelog-3-0-8-carries-a-second-unretracted-fixes-375-claim.md). The third — replace the instance-based P007 gates with a property assertion — is discharged by ADR-043 Confirmation 1, which **is** that property gate.
