...
| fact                                                          | count |
| ------------------------------------------------------------- | ----- |
| test files that read a repo file and assert on what they read | 44    |

<!-- MOVED 2026-09-19, 43 to 44. `managed-channel-rehearsal.test.mjs` entered, the first
     slice of the local dress rehearsal. It lands in the subject-EXERCISING group (15 to 16)
     because it spawns git: its corpus is `git ls-files --cached --others --exclude-standard`,
     chosen after a hand-maintained list of "generated" directories was caught wrong twice —
     once on vendored files carrying the literal it scans for, and once on a gitignored plan
     output that would have redded only on the machine of whoever last read a plan while
     staying green in CI. It lands in OTHER rather than workflow, and deliberately: the
     classifier tests the file's own source for two substrings, so citing the workflow tree in
     a justification comment would have been the sentence causing the fact it asserted. The
     read-only group is unchanged at 28. Figures taken from this guard's own failure messages
     across two runs, not hand-computed. -->

<!-- MOVED 2026-09-18, 41 to 42. `managed-channel-d1.test.mjs` began reading the
     migrations directory, to ENUMERATE which migrations to apply rather than to assert on
     their text. It hardcoded each migration before, so a new one was applied by no test at
     all until someone edited the helper. The predicate cannot see that distinction, which is
     why the file lands in the subject-exercising group (14 to 15) and not in read-only,
     which stays at 27. A file entering this census is not by itself evidence of the
     anti-pattern the census is named after. -->

| of those, files that read `.github/workflows/**` | 13 |
| all other file-reading test files | 31 |
...
`.github/workflows` while explaining the rule — so run without the exclusion the rule returns 45 / 14 / 31
...
The thirteen are `check-deps-is-time-bounded`, `deploy-guard-surfaces`, `docker-image-workflow`,
`liveness-corpus-covers-worker-cron`,
`gnaf-source-smoke`, `license-audit-runs-in-ci`, `loader-workflow`, `release-jobs-are-time-bounded`,
`release-pr-plan-workflow`, `release-workflow-deploy-only`,
`terraform-plan-workflow`, `terraform-required-vars-wired`
and `workflow-npm-scripts-resolve`.

**`managed-channel-health-workflow` LEFT this group on 2026-09-19 and the reason is worth
recording, because the file still does the thing the group is named for.** Its
`.github/workflows/...` path literal moved into a shared helper outside the tier, so it now
reads workflow YAML through a derived constant and its source no longer carries the two
substrings the published predicate tests. The predicate is textual and this is its known
imprecision, not a change in what the file does. It moved to the other group rather than
leaving the census.

**The first version of this list, as at 2026-08-08, said nine and omitted `license-audit-runs-in-ci`** (the correct answer was ten at that date; the list says thirteen today for unrelated reasons — the perf-probe deletion of 2026-08-20 and the two 2026-09-04 timeout bounds — so this sentence indicts the 2026-08-08 list, not the current one), because the predicate
...
Even now this over-counts the other group: `mutate-helper.test.mjs` is among the 31 and holds no source pin
...
**The conclusion that survives, and the one that does not.** Those thirteen files pin YAML consumed by GitHub.
Nothing in this repo runs a workflow, so extract-and-feed cannot convert those PINS and the honest
remedy is the note-what-it-cannot-establish task below. Said of the pins, not of the files — four of the
thirteen spawn a runtime for something else, so the older wording "there is no runtime in this repo to feed
them" was true of nine files and is corrected below. **Restated 2026-08-20 (second pass):** this paragraph
...
one fact, corrected in the ticket that exists to name that defect — the fifth such correction here.
**Restated 2026-09-04 (third pass), and this time the sentence is COMPUTED:** it still read "three of the
eleven … true of eight" after the check-deps bound moved the workflow group 11 → 12. The guard that fired on
every other cardinal in this ticket was blind to this one, because it asserts the phrases it was given and
this pair of numbers was not among them. Correcting it by hand a third time would have earned a fourth, so
`p033-population-figures-recompute.test.mjs` now derives both numbers from the same predicate as the rest. That is a claim about what those files ARE, and it
...
- [x] **Audit all `test/js/__tests__/*.test.mjs` files. DONE 2026-08-19: thirteen source pins read
      ...
      **Step 1 — 44 files read a repo file and assert on what they read.** Of those, **16 exercise the
                                                                                                                                                                                                                                              subject** (they import it, statically or dynamically, or spawn it) and **28 read only**. The first
                                                                                                                                                                                                                                              ...
                                                                                                                                                                                                                                              **Step 2 — of the 28 read-only files, NONE is a source-inspection test of implementation.** Nine
                                                                                                                                                                                                                                              read `.github/workflows/**`, where there is no runtime to exercise. The other nineteen check declarative
                                                                                                                                                                                                                                              ...
                                                                                                                                                                                                                                              **What this audit does NOT establish.** Three limits, the third of which was written because the
                                                                                                                                                                                                                                              ...
                                                                                                                                                                                                                                              3. **Which cardinals here are computed, and which are read.** Computed and mutation-proved by
                                                                                                                                                                                                                                                 `p033-population-figures-recompute.test.mjs`: the 34 / 13 / 21 Step 1 split, the named eleven-file list, the
                                                                                                                                                                                                                                                 un-excluded triple (35 / 12 / 23), the three intersections (eight, thirteen, three), and the Step 4 table's own
                                                                                                                                                                                                                                              ...
                                                                                                                                                                                                                                              One correction to an earlier claim while these figures are being reconciled: 12 workflow-reading files
                                                                                                                                                                                                                                              minus 9 read-only means **four already spawn a runtime**, so "there is no runtime in this repo to
                                                                                                                                                                                                                                              feed them" holds of the read-only remainder and not of the whole group. Applied, not just noted: the
                                                                                                                                                                                                                                              two sites that carried that wording are now phrased about the workflow PINS, for which the claim holds
                                                                                                                                                                                                                                              of every file in the group. Recording the correction and leaving the sites standing is this ticket's
                                                                                                                                                                                                                                              failure mode 4, and it is what happened on the first pass. Figures restated 2026-08-20 when the perf
                                                                                                                                                                                                                                              probe's deletion removed one workflow-reading file, and again 2026-09-04 when the check-deps bound
                                                                                                                                                                                                                                              added one; the three-spawn-a-runtime finding is unchanged by both. **This paragraph is now written
                                                                                                                                                                                                                                              WITHOUT restating either moving cardinal in words**, deliberately: it had to be hand-corrected on both
                                                                                                                                                                                                                                              of those passes, and a sentence that says "the read-only remainder" instead of a number cannot go stale
                                                                                                                                                                                                                                              when the population moves. The two numbers that do still appear are guarded.
