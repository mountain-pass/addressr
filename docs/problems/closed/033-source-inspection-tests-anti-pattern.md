...
| fact                                                          | count |
| ------------------------------------------------------------- | ----- |
| test files that read a repo file and assert on what they read | 43    |
...
| all other file-reading test files | 29 |
...
`.github/workflows` while explaining the rule — so run without the exclusion the rule returns 44 / 15 / 29
...
Even now this over-counts the other group: `mutate-helper.test.mjs` is among the 29 and holds no source pin
...
- [x] **Audit all `test/js/__tests__/*.test.mjs` files. DONE 2026-08-19: thirteen source pins read
      ...
      **Step 1 — 43 files read a repo file and assert on what they read.** Of those, **15 exercise the
