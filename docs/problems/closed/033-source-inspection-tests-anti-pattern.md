...
| fact                                                          | count |
| ------------------------------------------------------------- | ----- |
| test files that read a repo file and assert on what they read | 35    |
| of those, files that read `.github/workflows/**`              | 12    |
...
`.github/workflows` while explaining the rule — so run without the exclusion the rule returns 36 / 13 / 23
...
The twelve are `check-deps-is-time-bounded`, `deploy-guard-surfaces`, `docker-image-workflow`,
`gnaf-source-smoke`, `license-audit-runs-in-ci`, `loader-workflow`, `release-pr-plan-workflow`,
`managed-channel-health-workflow`, `release-workflow-deploy-only`, `terraform-plan-workflow`,
`terraform-required-vars-wired`
and `workflow-npm-scripts-resolve`.

**The first version of this list, as at 2026-08-08, said nine and omitted `license-audit-runs-in-ci`** (the correct answer was ten at that date; the list says twelve today for unrelated reasons — the perf-probe deletion of 2026-08-20 and the 2026-09-04 check-deps bound — so this sentence indicts the 2026-08-08 list, not the current one), because the predicate
...
**The conclusion that survives, and the one that does not.** Those twelve files pin YAML consumed by GitHub.
...
twelve spawn a runtime for something else, so the older wording "there is no runtime in this repo to feed
...
- [x] **Audit all `test/js/__tests__/*.test.mjs` files. DONE 2026-08-19: thirteen source pins read
      ...
      **Step 1 — 35 files read a repo file and assert on what they read.** Of those, **13 exercise the
                                                                                                                                                                                          subject** (they import it, statically or dynamically, or spawn it) and **22 read only**. The first
                                                                                                                                                                                          ...
                                                                                                                                                                                          **Step 2 — of the 22 read-only files, NONE is a source-inspection test of implementation.** Nine
                                                                                                                                                                                          read `.github/workflows/**`, where there is no runtime to exercise. The other thirteen check declarative
