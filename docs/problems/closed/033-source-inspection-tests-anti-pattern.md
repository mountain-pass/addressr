...
| fact                                                          | count |
| ------------------------------------------------------------- | ----- |
| test files that read a repo file and assert on what they read | 41    |
| of those, files that read `.github/workflows/**`              | 14    |
| all other file-reading test files                             | 27    |
...
`.github/workflows` while explaining the rule — so run without the exclusion the rule returns 42 / 15 / 27
...
Even now this over-counts the other group: `mutate-helper.test.mjs` is among the 27 and holds no source pin
...
- [x] **Audit all `test/js/__tests__/*.test.mjs` files. DONE 2026-08-19: thirteen source pins read
      ...
      **Step 1 — 41 files read a repo file and assert on what they read.** Of those, **14 exercise the
