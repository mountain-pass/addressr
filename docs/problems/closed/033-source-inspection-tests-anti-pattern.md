...
| of those, files that read `.github/workflows/**` | 14 |
| all other file-reading test files | 30 |
...
`.github/workflows` while explaining the rule — so run without the exclusion the rule returns 45 / 15 / 30
...
The fourteen are `check-deps-is-time-bounded`, `deploy-guard-surfaces`, `docker-image-workflow`,
...
`release-pr-plan-workflow`, `managed-channel-health-workflow`, `release-workflow-deploy-only`,
`terraform-plan-workflow`, `terraform-required-vars-wired`
and `workflow-npm-scripts-resolve`.

**The first version of this list, as at 2026-08-08, said nine and omitted `license-audit-runs-in-ci`** (the correct answer was ten at that date; the list says fourteen today for unrelated reasons — the perf-probe deletion of 2026-08-20 and the two 2026-09-04 timeout bounds — so this sentence indicts the 2026-08-08 list, not the current one), because the predicate
...
Even now this over-counts the other group: `mutate-helper.test.mjs` is among the 30 and holds no source pin
...
**The conclusion that survives, and the one that does not.** Those fourteen files pin YAML consumed by GitHub.
...
fourteen spawn a runtime for something else, so the older wording "there is no runtime in this repo to feed
them" was true of ten files and is corrected below. **Restated 2026-08-20 (second pass):** this paragraph
...
- [x] **Audit all `test/js/__tests__/*.test.mjs` files. DONE 2026-08-19: thirteen source pins read
      ...
      **Step 1 — 44 files read a repo file and assert on what they read.** Of those, **16 exercise the
                                                                                                                                                                                                                                          ...
                                                                                                                                                                                                                                          **Step 2 — of the 28 read-only files, NONE is a source-inspection test of implementation.** Ten
                                                                                                                                                                                                                                          read `.github/workflows/**`, where there is no runtime to exercise. The other eighteen check declarative
