...
- The three failures share a pattern: **the running addressr-server's actual behaviour diverges from the configured intention with no observable failure surface that the operator routinely checks**. Each failure mode required ad-hoc diagnostic probing initiated for a different reason to surface.
