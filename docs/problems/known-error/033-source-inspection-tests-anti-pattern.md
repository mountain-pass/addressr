...
- [ ] **Convert the remaining shell-predicate pins.** **The decision-bearing pins are DONE as of
      ...
      **No cardinal is restated here** — an earlier revision said "22 files" and went stale within a day. The
                              ...
                              **ONE file still reads a production shell script and asserts on its text**, as of
                              2026-08-21: `release-workflow-deploy-only.test.mjs`, at 22 assertions — 13 over
                              `scripts/release-watch.sh`, 8 over `scripts/push-and-watch.sh`, and one `doesNotMatch`
                              over `scripts/scan-jobs.awk`. It needs the shell-predicate extraction the awk scan
                              already demonstrates: pull the predicate into a file, feed it inputs, assert the exit
                              code. `deploy-artefact-ignores` and `terraform-plan-workflow` converted 2026-08-21 (rows
