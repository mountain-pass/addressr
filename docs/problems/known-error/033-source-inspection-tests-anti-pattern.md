...
- [x] **Audit all `test/js/__tests__/*.test.mjs` files. DONE 2026-08-19: thirteen source pins read
      ...
      **Step 1 — 32 files read a repo file and assert on what they read.** Of those, **15 exercise the
                                          ...
                                          **Step 2 — of the 17 read-only files, NONE is a source-inspection test of implementation.** Six
                                          read `.github/workflows/**`, where there is no runtime to exercise. The other eleven check declarative
                                          ...
                                          **What this audit does NOT establish.** Three limits, the third of which was written because the
                                          ...
                                          3. **Which cardinals here are computed, and which are read.** Computed and mutation-proved by
                                             `p033-population-figures-recompute.test.mjs`: the 32 / 15 / 17 Step 1 split, the named nine-file list, the
                                             un-excluded triple (33 / 10 / 23), the three intersections (six, eleven, three), and the Step 4 table's own
