...
- [ ] **Convert the remaining shell-predicate pins.** **The decision-bearing pins are DONE as of
      ...
      **No cardinal is restated here** — an earlier revision said "22 files" and went stale within a day. The
                                  ...
                                  **ONE file still reads a production shell script and asserts on its text**, as of
                                  2026-08-21: `release-workflow-deploy-only.test.mjs`, at 21 assertions — 13 over
                                  `scripts/release-watch.sh` and 8 over `scripts/push-and-watch.sh`. **Was 22; the `scan-jobs.awk`
                                  half discharged 2026-08-21** — a `doesNotMatch` asserting the scan had not been weakened to let
                                  pending jobs pass, plus an `existsSync` on the file. Both were redundant against
                                  `scan-jobs-awk.test.mjs`, which drives the real awk over 15 fixtures, and the redundancy was
                                  MEASURED rather than assumed: inserting the pending-passes rule is CAUGHT there, deleting the file
                                  is CAUGHT there, and removing the `skipped` clause is CAUGHT there while the text pin was BLIND to
                                  it. Strictly stronger, not merely equal — so the pins came out subtractively. **The 21 that remain
                                  are a materially larger job than the `deploy.sh` conversion was**: `release-watch.sh` is 509 lines
                                  with 25 `gh` invocations, 13 `git` and 12 `npm`, so the stub surface is a canned GitHub API rather
