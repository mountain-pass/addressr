...
### Implementation notes — the ADR-045 changeset guard (2026-08-17)

These are notes on HOW criterion 5 was built, recorded here rather than in ADR-045 because an ADR is a single decision and implementation detail that rides on its ratification without being ratified itself is how amendments rot. This ticket is the natural home: **Symptom 3 is verbatim the failure the guard catches** — infra committed but not applied, sitting on master until an unrelated publish carries it out as a rider.

**Shape.** One shared predicate, `scripts/check-deployment-changeset.sh`, called by two surfaces: `.husky/pre-push` and `.github/workflows/deploy-guard.yml`. Sharing the predicate is the point — two implementations of "will anything bump the deployment package" would drift, and the drift would be invisible until an apply silently did or did not happen.

**Decisions taken during implementation, each of which could have gone the other way:**

1. **Scoped to `refs/heads/master` in the two CALLERS, not in the predicate.** JTBD-400 sanctions dispatching the plan workflow against a short-lived branch; that push is by construction a new remote branch (all-zeros parent), carries files under the watched tree, and usually has no changeset yet. Guarding it would have reddened a documented workflow on day one — the measurable form of ADR-045's own "a guard people bypass is worse than none" criterion. Scoping to master dissolves it by construction rather than by carve-out and costs no coverage, because infra can only sit unapplied once it reaches master.

2. **Pending changesets are read from the HEAD TREE OBJECT** — not from the range diff, and not from the working tree. Reading the range would miss a changeset that landed in an earlier push and is still pending, reding a well-declared follow-up (the mixed-push false red that falsifies the rule). Reading the working tree would let an uncommitted `.changeset/*.md` pass the hook while the pushed commits carry nothing — P011 (`ef66d39`) reappearing inside the guard built to catch it, with the two surfaces disagreeing, hook green and CI red.

3. **An unresolvable parent does NOT block when a qualifying changeset is already pending.** The rule is "BLOCK when nothing pending will bump the deployment package". An unresolvable parent means you cannot determine what CHANGED; it does not mean you cannot determine what is PENDING. Blocking there would red an author who did everything right. This is strictly a subset of the fail-closed blocks and lets through nothing the guard exists to catch, because in the Terraform-change-with-no-changeset case the positive evidence is absent by construction.

3a. **The residual that refinement deliberately leaves standing: the push that CREATES `master` is refused.** Recorded beside note 3 because it is the same branch resolved in the opposite direction, and because without this note the first person to hit it reads it as a bug in the fail-closed branch and "fixes" it. An all-zeros parent with infrastructure present and nothing pending blocks. This cannot arise on `addressr` — master exists, so every push carries a real parent — but it would on a fresh clone-and-recreate. It is kept rather than exempted for three reasons. First, the exemption would have to key on the one signal that cannot be distinguished: "creating this ref" looks identical whether it is a benign fresh clone or a recreation after a history rewrite or a branch deletion, and the second is a case where infrastructure genuinely can arrive on master unarmed and unnoticed — symptom 3 exactly. Second, a zeros-parent exemption is a carve-out inferred from the state of the remote, recorded nowhere and reviewable by nobody; that is structurally the `workflow_dispatch` boolean the persona already ruled out, expressed as an `if`. Third, the cost it would buy relief from is near zero and non-recurring: the refusal names the trailer and names the reason it applies — "for a recovery push whose parent ref cannot be resolved", which is this case — so the operator meets the artefact once, while doing something extraordinary and paying attention. The carve-out's cost would be permanent.

4. **The cascade set is DERIVED from the deployment manifest's own dependency block**, intersected with the workspace package set from the root `workspaces` globs — the same source of truth changesets reads to compute the cascade, so the guard cannot answer a different question than the release will. Hardcoding `@mountainpass/addressr` would have been a second statement of the cascade set. It **fails loud rather than empty**: a silently empty cascade set would degrade the guard into always-require-a-deployment-changeset, one of the three shapes criterion 5 explicitly rejected.

5. **A third exclusion beyond the two criterion 5 names.** The trigger set excludes `package.json` and `*.md` per the criterion, and also `*.test.js`: `apps/addressr-deployment/cloudflare-worker/worker.test.js` sits under the tree but is not bundled by the worker and cannot reach production, so demanding a declaration for it would require arming a change that arms nothing.

6. **`--no-renames` is kept but is NOT what makes a tree move visible**, contrary to what the design review asserted and I initially wrote. Replayed against a real `git mv`: with a pathspec limiting the diff to the watched directory, `git diff --name-only` prints the SOURCE path with or without the flag. The PATHSPEC does the work. The flag is retained as explicit intent and as cover for a future edit that drops or widens the pathspec. Recorded because a comment claiming a flag is load-bearing when it is not invites a correct "this does nothing" cleanup that teaches the wrong lesson about the next flag.

**The refusal message is treated as the deliverable**, not as decoration on the rule, per JTBD-400's "checkable artefacts, not memory" outcome. It names the package literally, the command, the consequence of ignoring it, and the bypass trailer last with its purpose. It deliberately does **not** name `deploy_only`: criterion 5's sequencing paragraph requires naming it only if the guard shipped ahead of the gate, and it shipped behind — the input was deleted at `8199e5b9`. That sentence is still live in the ADR, so an implementer working from the text alone would point authors at a dispatch GitHub now rejects. Its absence is asserted in test.

**P052 interaction, recorded so it is not rediscovered mid-incident.** `@windyroad/risk-scorer`'s `check_ci_status` denies a push whenever master's latest CI run concluded `failure`, with no ancestor carve-out. A red `deploy-guard.yml` can therefore block the very follow-up push that adds the missing changeset, forcing `git push --no-verify` — which also drops the risk-score and external-comms gates. This is why the refusal names both recovery routes explicitly.

**Verification.** 47 new tests (`check-deployment-changeset.test.mjs`, `deploy-guard-surfaces.test.mjs`), driving real throwaway git repositories and, for the hook, a real push to a real local bare remote rather than a read of the file's shape — the old `.husky/pre-push` looked fine and could not block, because its single `|| echo` line guaranteed exit 0. Mutation-verified: rename detection, working-tree reads, range-only reads, harmonising the exit-code contract with the sibling script, dropping the cascade fail-loud, tip-only trailer scanning, naming `deploy_only`, dropping the master scoping, dropping `set -e`, moving the guard out of final position, dropping `fetch-depth: 0`, sharing `release.yml`'s concurrency group, and adding a `pull_request` trigger each turn the suite red. One mutation initially SURVIVED — the cascade fail-loud — because no test reached that branch; the case was added and the branch is now pinned.
