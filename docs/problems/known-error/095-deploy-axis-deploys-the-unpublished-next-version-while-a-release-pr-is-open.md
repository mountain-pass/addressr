...
- [ ] **Register curation — attempted, reverted, and bigger than it looks.** Two entries carry claims this fix falsifies: the publish/deploy-coupling entry states `deploy/deploy.sh` derives `elasticapp_version` from `npm_package_version` (now false), and the push-tier-apply entry's base rate reads "four production applies, all successful" (now five, one failed) while its own Monitoring names that failure as the trigger that "moves likelihood off Rare immediately" — which would take its residual above appetite.
  ...
  A two-line correction was written and reverted, because `risk-register-invariants.test.mjs` failed it: **five other entries reference those two and must be revisited when they change**, and the apply count is a canonical cell mirrored in three further places within the push-tier entry alone. That is the register's anti-drift machinery working exactly as intended, and it makes this M-effort curation rather than an edit to ride on a fix commit. Left tracked here rather than half-done.
...
- [P094](../open/094-published-package-with-geo-enabled-is-tested-by-nothing.md) — both are gaps between what CI exercises and what production actually runs.
