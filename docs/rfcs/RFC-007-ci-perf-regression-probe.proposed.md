...
- [ ] Close the wrong-measurement residual left open by the 2026-07-26 exit-code discrimination change: a probe that runs but measures the wrong thing breaches `checks{phase:main}` and so exits 99, landing in the advisory branch alongside a genuine perf regression. Route on run *content* rather than `$?` — either a grep of the tee'd log or a `handleSummary` JSON export. See the P032 residuals section.
