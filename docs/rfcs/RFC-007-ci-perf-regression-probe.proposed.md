...
3. **`.github/workflows/perf-regression.yml`** — a SEPARATE workflow (NOT folded into `release.yml`'s matrix, NOT per-push): `workflow_dispatch` + nightly `schedule`. Spins up an OpenSearch 3.5.0 service (the production engine), generates `version.js`, prepares the slim OT G-NAF fixture (same steps as `release.yml` build-and-test), loads it (`node loader.js`, `ES_INDEX_NAME=ot COVERED_STATES=OT`), starts the API server (`node src/server2.js` on `:6060`), waits for `/health`, then runs the probe. (Corrected 2026-08-09: both commands read `babel-node` until ADR 044 retired Babel on 2026-08-08. This RFC is proposed and its workflow does not exist yet, so the stale form was a forward instruction that would have been transcribed faithfully by whoever implemented it — the reason it was corrected rather than left as a dated note.)
...
- [ ] (Architect follow-on) Record a proposed ADR capturing the standing perf-regression methodology (direction pinned same-turn per ADR-064), including the exit-code discrimination rule and its known residual.
- [ ] Close the wrong-measurement residual left open by the 2026-07-26 exit-code discrimination change: a probe that runs but measures the wrong thing breaches `checks{phase:main}` and so exits 99, landing in the advisory branch alongside a genuine perf regression. Route on run _content_ rather than `$?` — either a grep of the tee'd log or a `handleSummary` JSON export. See the P032 residuals section.
