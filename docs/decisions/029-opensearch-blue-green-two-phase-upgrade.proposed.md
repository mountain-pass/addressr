...
5. **Populate the v2 domain**. Trigger `.github/workflows/populate-search-domain.yml` via `workflow_dispatch` with `target: v2` and `states: all` (or a comma-separated subset for spot loads). The workflow reuses `reusable-update.yml` — the same loader path the quarterly `update-{state}.yml` workflows use — pointed at the v2 secrets via the `target` input. Each matrix leg `npm install`s the published `@mountainpass/addressr` and runs `addressr-loader`. The latest published loader is verified compatible with engine 2.19.x by the dual-version CI matrix on commit `d3d1e09` (`build-and-test (1.3.20)` + `build-and-test (2.19.5)`); if a future Phase needs a code change to support the target engine, publish first then trigger populate. Default sequential (`max-parallel: 1`) per state to respect the "scheduled outside peak traffic windows" Consequence; operator may edit the workflow's matrix strategy before triggering for parallel load. While population runs, `search-addressr3-…` continues to serve production via the unchanged `update-{state}.yml` quarterly schedules (which inherit `target=v1` default).
   ...
   > **Step 5 amendment 2026-05-13** — populate-window v2 sizing is **decoupled from steady-state v2 sizing**. The original "match `search-addressr4-…`'s instance class to `search-addressr3-…`'s" (Consequences line 106) was conceived as a steady-state cost-mitigation hedge. In practice v1's sizing works because v1 only serves steady-state queries; quarterly G-NAF refreshes against v1 are small deltas. v2's populate is a full from-scratch 16.8M-doc bulk-index concurrent with shadow soak queries — a workload v1 never carries. populate run 25731879773 hit `snapshot_in_progress_exception` on the close-index step for QLD + WA (I001); the t3.small-class retry run 25762661760 hung 2h+ on QLD with shadow success rate degrading from 95% → 52% under bulk-index contention. Therefore: scale v2 up during the populate window (`m6g.large.search` × 2, 20 GB EBS) and scale back to the v1-matching class (`t3.small.search` × 2, 10 GB EBS) post-populate. The scale-back is tracked by **P038**. The resize reset the ADR 031 soak gate — `/debug/shadow-config` must show clean 2xx rate post-resize before any new 48h soak window starts counting (per ADR 031 Confirmation amendment 2026-04-29). Decision Outcome unchanged; this is a populate-window operational property, not a steady-state cost change.

   > **Phase 1 re-attempt 2026-07-06 (supersedes the step 5 amendment 2026-05-13 above)** — Phase 1 is re-attempted with revised sequencing and sizing, per the user-approved plan of 2026-07-06. Changes from the first attempt:
   >
   > 1. **Quiet-populate-first sequencing.** The v2 domain is provisioned with NO `ADDRESSR_SHADOW_*` env vars — it receives zero query traffic while `populate-search-domain.yml` loads all states. Read-shadow (ADR 031) is enabled only after populate completes and validates. This separates the populate window from the warming window, removing the bulk-index-vs-shadow contention that drove I001 and the 2026-05-13 scale-up.
   >    **Sizing amendment 2026-07-08 (from-scratch-load finding).** The first re-attempt load reproduced the P035 index deletion at 14M/16.8M docs on `t3.small` × 2 + 12 GB — but the diagnosis is now clear and it is **not** an instance-class-serving limit: v1 holds the full 16.8M on the _identical_ t3.small × 2 + 12 GB at ~56% disk, **and v1 was itself loaded from scratch at creation on 1.3.20** (operator recollection 2026-07-08 — correcting an earlier draft that wrongly attributed it to incremental growth). So this is a **2.19 engine issue, not a load-pattern one**: the same t3.small survived a from-scratch load on 1.3.20 but not on 2.19, which is materially heavier — **~1.7× the disk-per-doc of 1.3.20** (v2 hit ~80% of 12 GB at 14M) plus a heavier heap/indexing profile. The from-scratch bulk-index of the full set on the heavier 2.19 engine overwhelms the t3.small (node drop → index deletion), and 16.8M with a replica won't fit 12 GB on 2.19. **Fix (user-confirmed 2026-07-08): EBS 12 → 20 GB, and load with `replicas: 0` (via an `addressr`-matching index template) to halve disk + write pressure, then add the replica post-load.** Instance class stays `t3.small` (v1 proves it _serves_ 16.8M fine). P035 is confirmed FGAC-independent and loader-independent (ADR 033 honesty caveat validated); the doc-count alarm caught the deletion within minutes. Escalation if this still fails: `m6g.large` for the load, or load-big-snapshot-restore-to-small.
   > 2. **Steady-state sizing; never resize the instance class.** v2 provisions at `t3.small.search` × 2 + 20 GB gp3 (EBS bumped per the 2026-07-08 amendment above) and the instance class stays there through cutover. The 2026-05-13 populate-window scale-up + P038 scale-back guidance is superseded: resize blue/green is what stuck for 3+ hours and triggered the rollback, and 2 of 3 FGAC-clobber observations (P036) followed AWS-side reconfigurations. Escalation if populate fails on t3.small: **destroy + recreate** at `m6g.large.search` × 2 + 20 GB (recreate proved safe on 2026-05-12; resize did not), with fresh-creds rotation per the ADR 030 distinct-credentials note.
   >    2a. **One-off in-place resize-to-`m6g.large.search` parity TEST (2026-07-08, user-approved) — deviation from point 2's never-resize rule, NOT the cutover path.** The full-dataset warm parity read showed v2-on-t3.small materially slower than v1 (warm p50 ~186 ms vs ~50 ms; p90 in the seconds). CloudWatch diagnosis: v2 CPU (~10%) and JVM (~50%) are _lower_ than v1 (~18%/~65%) at the same query rate, so v2 is **I/O-bound** (page-cache misses — the 2 GB t3.small cannot hold the hot-set of the ~1.7×-larger 2.19 index), not compute-bound. m6g.large (8 GB) gives the RAM to hold it. The test resizes v2 in place, warms via read-shadow, and measures m6g.large-vs-v1; the keep-vs-revert sizing outcome is decided on the number, deferred here per ADR-074 (do not cement an unmeasured choice). **Why resize is acceptable for this bounded test despite point 2:** (i) the FGAC-clobber driver (P036) is structurally removed by ADR 033 — no `.opendistro_security` index exists to clobber; (ii) v2 takes zero production traffic (`ELASTIC_HOST`=v1), so the resize-stall risk (the OTHER never-resize driver, which ADR 033 does NOT remove) costs only time, not an outage — destroy+recreate stays staged as the ready fallback and the resize is time-boxed; (iii) P035 (silent index deletion) is NOT fixed by ADR 033 and a resize IS the control-plane reconfiguration class it warned about, so it could recur — the `addressr-v2-searchable-documents-drop` alarm was confirmed live (OK, threshold 1M, docs 16,905,824) before apply, so a recurrence trips within minutes and a wipe on the non-serving domain = reload, not outage. If the test keeps m6g.large, point 2 is amended to bless resize/recreate+m6g.large steady-state; if it reverts, this note records the tested-and-reverted outcome with no lasting change.
   >
   > **Result (2026-07-09): m6g.large PROVED parity — steady-state sizing still under measurement.** After the resize, addressr4 warmed cleanly (no stall, no P035 wipe — ADR 033's FGAC removal held) and reached parity with v1, slightly ahead in the warmed hour: v2 `SearchLatency` p90 45 ms / p99 115 ms vs v1 p90 64 ms / p99 187 ms. This confirms the RAM hypothesis — the t3.small slowness was I/O-bound (2 GB can't hold the ~1.7× 2.19 hot-set), not a 2.19 engine regression; adequate memory puts the tail below v1. The user then chose (2026-07-09) to **revert to t3.small.search and run a longer (48 h) soak** to measure empirically whether t3.small can reach parity with full cache warming before committing to m6g.large's ~$200/month. So there are now **two measurements feeding the deferred steady-state sizing choice**: m6g.large = parity proved; t3.small = pending the 48 h number. The keep-vs-revert-vs-recreate sizing decision remains **open** on the t3.small soak result (do not read the revert as a closed "t3.small chosen" — it is a measurement, not a commitment); per ADR-074 the steady-state sizing is recorded only when that number lands.
   >
   > **CLOSED (2026-07-09, user-confirmed via AskUserQuestion): steady-state sizing for 2.19 = `m6g.large.search` × 2.** The t3.small soak answered the deferred question decisively — t3.small does NOT reach parity with more warming; under real load it _diverged_ from v1 (p90 climbed 1,156 → 2,753 ms and rising over 4h while v1 held ~200 ms; p99 ~5-8 s). That is a memory/capacity ceiling (2 GB cannot hold the ~1.7× 2.19 hot-set), not a warming lag — soak time does not fix it. m6g.large proved parity and slightly ahead (warm p90 45 ms / p99 115 ms vs v1 64 / 187). **This amends point 2 above**: the never-resize / t3.small-from-start rule is superseded by measurement — the steady-state class for the 2.19 domain is `m6g.large.search` × 2. In-place resize is now an accepted mechanism alongside destroy+recreate: 3 resizes ran clean this cycle and ADR 033's FGAC removal eliminated the P036 clobber that stalled the original 2026-05 resize (the resize-stall risk is bounded by v2's zero prod traffic, with destroy+recreate staged as fallback). The Decision Outcome (two-phase blue/green, 2.19-first, reindex-from-GNAF, zero-outage) is unchanged; only the Phase 1 instance-sizing sub-point changed. Next: warm the resized m6g.large, run the formal k6 gate (ADR 031), then cutover (Stage 5).
   >
   > **Zero-outage hardening (2026-07-10): `/health` now probes OpenSearch.** The cutover's zero-outage guarantee relies on EB's health-gated rolling deploy + `RollbackLaunchOnFailure` reverting a bad cutover before it reaches consumers. But `/health` was static (`{status:'healthy'}` without touching OpenSearch), so a misconfigured v2/SigV4 cutover would have passed health, rolled out fleet-wide, and NOT auto-rolled-back — users would get query errors the safety net couldn't see. Fixed: `/health` (via `src/es-health.js` `checkEsHealth`) now `ping()`s the app's OpenSearch client and returns **503** when it is unreachable/mis-authed, **200** when live. `ping` (HEAD /) is used over a GREEN-keyed `cluster.health()` so it does not false-red during the legitimately-yellow replicas-0-then-add-replica populate window, and over a `count` (heaviest, and an abuse-amplification vector since `/health` is on the ADR 024 unauthenticated allowlist). Transient-blip tolerance lives in the ELB thresholds (`UnhealthyThreshold=5` consecutive failures at the configured `Interval=10s` ≈ **~50 s** sustained), which also absorb the brief startup-connect window (server2 listens before `globalThis.esClient` latches; `esConnect` completes in a few seconds on a fresh instance, well under ~50 s) — so a healthy deploy is never false-failed while a sustained bad-cutover 503 trips rollback. An `HEALTH_ES_PROBE=off` env kill-switch reverts `/health` to always-200 without a redeploy if the new ES→ELB-pool coupling ever misbehaves in production. Verified end-to-end: live v2 → 200 healthy; unreachable endpoint → 503 not-connected; unit 6/6 + full nodejs Cucumber vs real v2 13/13. Bounded implementation detail under this ADR's zero-outage constraint (architect-reviewed 2026-07-10; Decision Outcome unchanged, no re-confirmation). 3. **Why not the rollback note's m6g.large-from-start suggestion:** quiet populate removes the shadow contention that overwhelmed t3.small last time, and the P037 loader fix (initIndex fast-path + snapshot_in_progress retry, shipped ahead of populate) removes the I001 snapshot-race failure class. Deviating is a reasoned choice, user-confirmed 2026-07-06, avoiding ~$200/month of permanent steady-state cost; destroy+recreate remains the escape hatch. (Superseded in practice by point 2a's 2026-07-08 parity test: t3.small proved too small for 2.19 SERVING, so the cost-avoidance rationale is now under measurement.) 4. **Audit logs at provisioning** (P036 open task): the ADR 030 module now publishes `AUDIT_LOGS` to CloudWatch per domain, so any FGAC clobber recurrence is traceable. 5. **Parity monitoring is stood up against v1 BEFORE v2 spend** (Stage 0d): an `aws_cloudwatch_dashboard` in `deploy/main.tf` shows `SearchLatency`/`SearchRate`/`CPUUtilization` side by side for v1 and v2. The dashboard references the non-TF-managed v1 domain's metrics **by name only** — the ADR 030 scope boundary (no changes to the v1 resource) stays intact. The k6 pair remains the formal cutover gate per ADR 031.
   >
   > - **Domain-name correction (2026-07-07)**: the v1 domain NAME is `addressr3` — the `search-addressr3-…` string used throughout these ADRs is the AWS _endpoint_ prefix (`search-<name>-<hash>`), verified via `aws opensearch list-domain-names`. `var.elastic_v1_domain_name` defaults to `addressr3`. The v2 domain is named `addressr4` (renamed from attempt 1's literal `search-addressr4`, whose endpoint was the double-prefixed `search-search-addressr4-…`; the domain does not exist yet so the rename is free).
   > - **Statistic/period validated on v1 (2026-07-07)**: over the trailing week, hourly `SearchLatency` on `addressr3` is near-continuously present with per-hour sample counts sufficient for a meaningful percentile; hourly **p95** is well-populated (min/median/max ≈ 0/208/418 ms) with **Average** (median ≈ 55 ms) as the secondary line. (Sample-count figures are traffic volumes — confidential per RISK-POLICY; kept out of this public file deliberately.) Chosen: `p95 @ period 3600` primary, `Average @ 3600` fallback — as encoded in the dashboard. CloudWatch `SearchLatency` is per-shard latency, NOT end-to-end request latency; it is used only for like-for-like v1-vs-v2 comparison, never against the k6 numbers. Because CloudWatch retains the metric history regardless of dashboard existence, the v1 baseline trend requirement is satisfied by the existing metric history — Stage 1 is not calendar-gated on dashboard age.
   > - **v1 k6 baseline refreshed (2026-07-07, run 2026-07-06 ~18:45–19:25 AEST, user-approved timing)**: search p95 **961.64 ms** (med 188.5 ms, avg 316.24 ms), retrieve p95 **123.92 ms** (med 58.58 ms), zero failed requests, 100% checks, full 38-min ramped profile against production v1 via a local server (same harness as 2026-04-29). This replaces the 741 ms number as the Stage 4 gate reference: **cutover requires v2 search p95 ≤ 1.5 × 961.64 ≈ 1,443 ms** on the matching k6 leg. Artifacts: `target/stress-v1-20260707.csv` + `-summary.json` (local, gitignored).
   >
   > 6. **Operator checklist for the provision step** (JTBD-400 — checkable artefacts, not memory). Before merging the module caller: (a) generate fresh v2 master creds (33+ chars, retaining URL-reserved characters to exercise the URL-encode fix); (b) store in 1Password "Addressr v2 OpenSearch"; (c) set GHA secrets `TF_VAR_ELASTIC_V2_USERNAME` / `TF_VAR_ELASTIC_V2_PASSWORD`; (d) dispatch `.github/workflows/sync-tfc-vars.yml` — it upserts the two TFC workspace variables from the GHA secrets and deletes stale `elastic_v2_*` overrides, so the GHA↔TFC match holds **by construction** on a green run (amendment 2026-07-07; the manual "confirm TFC matches" step was the P028 failure surface; three-plane TFC/GHA/EB divergence is the P028 failure mode). After apply: (e) copy the endpoint output into GHA secret `TF_VAR_ELASTIC_V2_HOST`; (f) verify cluster GREEN and a direct `_cluster/health` auth probe returns 200 with the 1P creds.
   >
   > P036 un-parks (its un-park trigger is this re-attempt). P038 stays parked; if t3.small populate succeeds it is permanently superseded per its own un-park trigger. `reassessment-date` bumped 2026-07-21 → 2026-09-30 (Stage 5 soak horizon).

   > **Phase 1 rolled back 2026-05-14** — this Phase 1 attempt is abandoned. The two-phase blue/green pattern remains the chosen long-term path (Decision Outcome unchanged; no supersession), but the current `search-addressr4` instance is decommissioned. **Trigger:** the m6g.large.search resize blue/green (per step 5 amendment 2026-05-13) entered stage-4 shard-copy and stuck for 3+ hours with non-monotonic AWS-managed telemetry (reported 10/34 → 34/34 → back to 10/34); cluster auth broken post-resize (3rd observation of the FGAC clobber pattern recorded in P036's standing-risk reframe 2026-05-13). **Surviving artifacts:** `./deploy/modules/opensearch/` retained without caller (ADR 030 Confirmation amendment 2026-05-14); `var.elastic_v2_*` variable declarations retained (deferred cleanup; orphaned-but-harmless); `src/read-shadow.js` capability shipped in default-off posture (ADR 031 amendment 2026-05-14). **What gets re-evaluated:** for a future Phase 1 re-attempt the operator should consider provisioning at the target instance class from the start (`m6g.large.search` × 2 + 20 GB EBS, or higher per data-volume estimate) rather than relying on resize blue/green; pre-stage rotation changesets alongside the resize/recreate per the prior P036 multi-deploy-recovery learning; and enable OpenSearch domain audit logs at provisioning time per the P036 standing-risk investigation task. **Tickets:** P036 and P038 close as superseded; I001 closes as restored (the populate failure surface no longer exists); P037 stays open (loader URL-encode + close-index race fixes apply to v1's loader path independently). Decision Outcome unchanged; the abandoned attempt is recorded, not the pattern.
...
- AWS cost double-up during the soak is a real line-item increase (managed OpenSearch is already the largest single AWS bill item per ADR 021). Mitigation: run Phase 1 overlap for the minimum viable soak and match `search-addressr4-…`'s **steady-state** instance class to `search-addressr3-…`'s. **Populate-window sizing is decoupled** — see step 5 amendment 2026-05-13 — because v2 must bulk-index the full dataset concurrent with shadow soak queries, a workload v1 never carries. Scale-back to the v1-matching class post-populate is tracked by P038.
