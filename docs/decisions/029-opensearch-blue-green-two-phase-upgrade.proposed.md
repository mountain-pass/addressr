...
- During Phase 1 (between step 1 and step 9), `.github/workflows/release.yml` `build-and-test` job has `strategy.matrix.opensearch_version` containing **both** `"1.3.20"` and `"2.19"` (or the chosen 2.19.x patch version), and `package.json` `config` carries both `SEARCH_IMAGE_1_3_20` and `SEARCH_IMAGE_2_19` entries.
