...
1. **Bump local + CI to 2.19**. Bump `SEARCH_IMAGE` in `package.json` to `opensearchproject/opensearch:2.19.x` and bump `services.opensearch.image` in `.github/workflows/release.yml` in lockstep. Verify `@opensearch-project/opensearch@^3.5.1` client compatibility against 2.19 (client matrix confirms 2.x support; no pin change expected — document if any). The existing `plugins.security.disabled: 'true'` env on the CI service container is expected to carry over; a CI-red on the service-container health check is the expected fail-fast surface for client/engine incompatibility and is the whole point of doing this step first.
2. **Run local validation against 2.19**. Cucumber suite + ADR 025 14-query symmetric-SSLA baseline against local `opensearchproject/opensearch:2.19`. This step also feeds the P027 (synonym-expansion bypassing `AUTO:5,8` fuzziness) co-investigation.
...
6. **Re-run validation against the AWS-managed v2 domain**. Cucumber suite + 14-query symmetric-SSLA baseline against `search-addressr4-…`. This catches AWS-managed-specific deltas (IAM/access-policy, TLS, instance class, data-node count) that local Docker cannot surface. Block cutover on any behavioural regression.
...
9. **Soak and decommission**. After a **soak of at least 7 days in production with no regressions**, decommission `search-addressr3-…`.
