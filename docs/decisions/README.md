...
**Decides:** Publish `mountainpass/addressr` as a two-stage build — `node:22-bookworm-slim` installs, `gcr.io/distroless/nodejs22-debian12:nonroot` runs — so the published artefact carries only the Node runtime, the app's dependency tree, and one vendored init, with no shell, package manager, or npm; uid 65532, a writable `/home/nonroot` WORKDIR, a resolved `CMD` script path, and re-declared `ELASTIC_*`/`ADDRESSR_INDEX_*` defaults preserve the runtime contract, while the in-container shell and the `docker run ... addressr-loader` command form are deliberately given up. Amended three times on 2026-07-26: a Debian-packaged `tini` runs as PID 1 after CI disproved this ADR's claim that node handles `SIGTERM` as PID 1 (ADR 013's signal-handling driver was right; graceful shutdown is deferred to P067, not dropped; bookworm/glibc stage parity is now load-bearing); ADR 040 moved tagging to `:<version>-<gitsha>` plus `:latest` with bare semver release-only, moved publishing into CI, and discharged the digest-pin trigger — a float that re-resolves on every rebuild beats a pin with no refresh mechanism; and a correction records that ADR 040 stage 2 shipped no reusable `workflow_call` definition and no `push` input, so build, smoke, and publish all stay in `docker-image.yml` job `build-and-smoke` with an inline `if:` as the pull-request guard.
**Confirmation:** `.github/workflows/docker-image.yml` job `build-and-smoke`: `npm run build:docker` completes; `docker inspect` reports a non-root user; the container answers a real HTTP request on `/health` (the check that catches an unresolvable `CMD` path); `docker stop` terminates in under 10s (this one failed at 11s and drove the `tini` amendment, and proves prompt termination only, not request draining); manually verify the loader writes `/home/nonroot/target/keyv-file.msgpack` without EACCES. The criteria-transfer note is moot — no reusable definition was created, so they stay pointed at `docker-image.yml` job `build-and-smoke`, where they already pass.
**Related:** ADR-011, ADR-013, ADR-015, ADR-040

### ADR-040 — Release Pipeline Decoupled into a Change-Type to Action Matrix

**Status:** proposed | **Oversight:** unconfirmed
**Decides:** Splits the release pipeline into three independently-triggered axes — npm publish on a consumed changeset, docker publish on image-relevant paths, and deploy on a release, a `deploy_only` dispatch, or a `deploy/**` push — so no axis is reachable only by laundering an unrelated version bump. Adds immutable `:<version>-<gitsha>` tags so a rebuild can never re-point a consumer's pin, and moves Docker publishing into CI behind two independent guards.
**Amended 2026-07-26 (stage 2, as built):** `docker-image.yml` keeps its push-to-master trigger and publishes on it (the input is `publish_semver`, not `push`); build/smoke/publish stay in one job rather than a reusable definition, so the pushed image is the smoke-tested image per ADR-039's compensating control; concurrency and `DOCKER_PUBLISH_SEMVER` move to job scope; the double-publish guard has no home yet and **blocks stage 3** until the user pins one of four reconciliations; stage 2 landed ahead of the ratification drain the Confirmation criteria required.
**Confirmation:** Ratified in the same drain as ADR-039; ADR-001 amendment lands before the `deploy/**` trigger is wired, asserted in test not by grep; `release-workflow-deploy-only.test.mjs` updated not deleted; `release` job exposes `published` via `needs.release.outputs`; path-detection scoping pinned in test for whatever stage 3 does with `published`; exactly one workflow line matching `run: npm run build:docker`; `docker-image.yml` header comment rewritten; `start:server:docker` resolves to a tag `build:docker` produces; a Dockerfile-only push publishes the image only; a release publishes npm plus one image plus deploy; a `deploy/**`-only push deploys only; the publish step skips green with secrets unset; a PR builds and smoke-tests without pushing; `:<version>-<gitsha>` resolves and the bare `:<version>` digest is unchanged.
