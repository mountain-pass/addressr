...
**Decides:** Publish `mountainpass/addressr` as a two-stage build — `node:22-bookworm-slim` installs, `gcr.io/distroless/nodejs22-debian12:nonroot` runs — so the published artefact carries only the Node runtime, the app's dependency tree, and one vendored init, with no shell, package manager, or npm; uid 65532, a writable `/home/nonroot` WORKDIR, a resolved `CMD` script path, and re-declared `ELASTIC_*`/`ADDRESSR_INDEX_*` defaults preserve the runtime contract, while the in-container shell and the `docker run ... addressr-loader` command form are deliberately given up. Amended twice on 2026-07-26: a Debian-packaged `tini` runs as PID 1 after CI disproved this ADR's claim that node handles `SIGTERM` as PID 1 (ADR 013's signal-handling driver was right; graceful shutdown is deferred to P067, not dropped; bookworm/glibc stage parity is now load-bearing), and ADR 040 moved tagging to `:<version>-<gitsha>` plus `:latest` with bare semver release-only, moved publishing into CI, and discharged the digest-pin trigger — a float that re-resolves on every rebuild beats a pin with no refresh mechanism.
**Confirmation:** `.github/workflows/docker-image.yml` job `build-and-smoke`: `npm run build:docker` completes; `docker inspect` reports a non-root user; the container answers a real HTTP request on `/health` (the check that catches an unresolvable `CMD` path); `docker stop` terminates in under 10s (this one failed at 11s and drove the `tini` amendment); manually verify the loader writes `/home/nonroot/target/keyv-file.msgpack` without EACCES. Criteria transfer unchanged to ADR-040's reusable `workflow_call` definition when stage 2 creates it; both locations are named so they stay checkable meanwhile.
**Related:** ADR-011, ADR-013, ADR-015, ADR-040

### ADR-040 — Release Pipeline Decoupled into a Change-Type to Action Matrix

**Status:** proposed | **Oversight:** unconfirmed
**Decides:** Splits the release pipeline into three independently-triggered axes — npm publish on a consumed changeset, docker publish on image-relevant paths (guarded by `published != 'true'` so a release cannot publish the image twice), and deploy on a release, a `deploy_only` dispatch, or a `deploy/**` push — so no axis is reachable only by laundering an unrelated version bump. Adds immutable `:<version>-<gitsha>` tags so a rebuild can never re-point a consumer's pin, moves Docker publishing into CI behind two independent guards, and keeps one definition per action via a reusable `workflow_call` workflow.
