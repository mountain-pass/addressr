...
**Total ADRs:** 43 (38 in-force, 5 historical)
...
**Decides:** Emit exactly two `sla_range_expanded` aliases per range doc — the `NUMBER_FIRST` and `NUMBER_LAST` endpoints — instead of ADR-026's full interpolation, because G-NAF records only those two as actual property addresses and interpolating mid-range numbers produced false positives (a `104 GAZE RD` query matching the `103-107` doc). Endpoint recall is preserved. Amended 2026-08-08 by ADR-043: the `phrase_prefix` ranking win is SUSPENDED, not surrendered — that clause is gone, so query-side this is ADR-028's own rejected Option D in the interim and endpoint recall rests entirely on the whitecomma tokenizer split. Index-side Option A is retained because the aliases are what recovers it: measured, giving them a `.raw` subfield and adding them to ADR-043's `dis_max` moves the range doc from #2 to #1 on this ADR's own TRAVEL INN case, with no asymmetry because `dis_max` maxes rather than sums. Pending a re-index, which is sequencing rather than an objection, storage shrinks, and `SPAN_CAP` is retired.
**Confirmation:** `expandRangeAliases(103, 107, ...)` returns exactly the two endpoint aliases with 104/105/106 absent; outlier span (1, 111015) still returns 2 elements; `sla_range_expanded` attachment tests hold unchanged; the query-side wiring assertions were re-pointed twice (2026-08-07 to behavioural assertions against `buildAddressSearchBody`, then 2026-08-08 onto ADR-043's `dis_max`), never deleted, so Reassessment Criterion 5 has never fired — though its absent-field rationale is now void and the pin rests on ADR-025 Driver 4; Cucumber first/last-endpoint recall for `103`/`107 GAZE RD` returns GAOT_717321171, `106` does NOT, canonical `103-107` still ranks first, v1 endpoint recall across nodejs/rest/cli; post-deploy diff against the v2.3.0 14-query baseline within enumerated bounds.
...
**Status:** proposed | **Oversight:** confirmed | **Supersedes:** ADR-042
**Decides:** Replaces the `match_phrase_prefix` clause with a `dis_max` over `prefix` queries on the `sla.raw`/`ssla.raw` keyword subfields, uppercased client-side and gated on query selectivity (a non-space followed by whitespace — the user has typed past the street number), so sub-units stop outranking their parent street address. Keeps ADR-042's diagnosis but drops its `span_first` mechanism, whose per-request `_analyze` round trip doubled p50 latency; keyword prefix reaches 0.0% street-level-first failure from a 60.0% baseline at one round trip, no re-index, and no engine-specific DSL — so ADR-042's ADR-025 Driver 3 override is withdrawn. Held at `proposed` after shipping in 3.0.8 until P074 closes and Confirmations 4 and 7 discharge.
**Confirmation:** Corpus-scale street-level-first on a randomly redrawn national sample; corpus-scale partial-prefix recall with the P078 sensitivity gate run against the `legacy` variant; fixture-scale Cucumber is non-regression only and discharges neither gate; ADR-027's 14-query baseline re-run post-deploy; query-body single source in `src/build-search-body.js`; ADR-028's five endpoint scenarios plus the mid-range false-positive and canonical-range-first probes now load-bearing; latency re-baselined in-run with replicates; short-prefix latency proves the selectivity gate; the uppercase assumption pinned at corpus, code and end-to-end layers, with the loader instrument recorded as an explicit unshipped gap; `dis_max` carries no explicit `tie_breaker` (on ADR-025 Driver 4, not absent-field grounds); `sla.raw`/`ssla.raw` carry no `ignore_above`; a boundary probe crosses the selectivity discontinuity; variant-tolerance floor as target-in-page, including single-character typos measured against the pre-decision query.
**Related:** ADR-021, ADR-025, ADR-026, ADR-027, ADR-028, ADR-029, ADR-031, ADR-041, ADR-042
...
**Status:** superseded | **Oversight:** confirmed | **Superseded by:** ADR-043
