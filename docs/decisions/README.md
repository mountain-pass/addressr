...
**Status:** proposed | **Oversight:** confirmed
**Decides:** Mirror production `/addresses` search and retrieve requests fire-and-forget to a second, configurable OpenSearch cluster (default-off behind `ADDRESSR_SHADOW_HOST`, basic or SigV4 auth) so a migration target is warmed by the real production query distribution before cutover. Chosen over query-replay, force-merge, and bigger instances because it is the only option that warms the caches customers actually hit, and it is reusable for the next migration; the ~0.5–1.0 ms primary-path cost was user-approved 2026-04-29.
**Confirmation:** `src/read-shadow.js` exports `validateReadShadowConfig` + `mirrorRequest`; ~12 DI-mocked unit tests in `test/js/__tests__/read-shadow.test.mjs`; `service/address-service.js` mirrors after the primary search/get without await; `src/waycharter-server.js` validates config at startup (sole call site post-ADR-036); Cucumber passes with shadow off; manual two-cluster probe shows shadow hits; primary-path invariant ≤ 1 ms p95 delta verified by back-to-back k6; soak gate rewritten 2026-07-31 to target-measured criteria — mirror-parity `query_total` growth with no SearchRate ALARM, zero shadow failures with sustained doc-count parity, `SearchLatency` p90 flattening within 10% across trailing 6 h, ≥24 h spanning a business-hours peak, and target p95 ≤ 1.5× a freshly re-derived primary baseline; soak-validity check probes the target directly, SigV4-signed under `sigv4` auth mode (P028).
**Related:** ADR-023, ADR-024, ADR-029, ADR-033, ADR-036
