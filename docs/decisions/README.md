...
**Confirmation:** `src/read-shadow.js` exports `validateReadShadowConfig` + `mirrorRequest`; ~12 DI-mocked unit tests in `test/js/__tests__/read-shadow.test.mjs`; `service/address-service.js` mirrors after the primary search/get without await; `src/waycharter-server.js` validates config at startup (sole call site post-ADR-036); Cucumber passes with shadow off; manual two-cluster probe shows shadow hits; primary-path invariant (1 ms p95) discharged 2026-07-31 by a controlled local A/B (+0.09 ms p95) plus a SigV4 signing microbenchmark (0.010 ms/signature) rather than the originally prescribed production k6 pair; soak gate rewritten 2026-07-31 to target-measured criteria — mirror-parity `query_total` growth with no SearchRate ALARM, zero shadow failures with sustained doc-count parity, `SearchLatency` p90 flattening within 10% across trailing 6 h, at least 24 h spanning a business-hours peak, and target p95 within 1.5x a freshly re-derived primary baseline (still owed pre-cutover); soak-validity check probes the target directly, SigV4-signed under `sigv4` auth mode (P028).
