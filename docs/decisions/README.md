...
### ADR-098 — The gateway admits loopback origins that Terraform cannot deploy
...
**Confirmation:** Three loopback admissions and nine refusals including the three contains-attacks, written red first; exact-port matching; the Terraform predicate EXECUTED against an isolated copy, accepted on https and refused on `http://127.0.0.1:9000` with its own message; and the validation's presence AND SUBJECT pinned at both sites by a test that runs, mutation-proved by pointing the condition at a different variable, because a pattern-only assertion survives that swap with the guard gone. Explicitly NOT satisfied: nothing asserts the shipped website bundle carries the production API base, and nothing asserts a job record's `screens:` path resolves — including the one naming this record's own pin test.
