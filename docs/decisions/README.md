...
**Decides:** Rebuild the published `mountainpass/addressr` image as a multi-stage build — `node:22-bookworm-slim` installs, `gcr.io/distroless/nodejs22-debian12:nonroot` runs — so a publicly pullable artefact carries only the Node runtime and the app's own dependencies: no shell, no package manager, no npm, no `dumb-init`. Non-root execution (uid 65532), a writable `/home/nonroot` WORKDIR, re-declared env defaults, and an unchanged `npm run build:docker` carry over; the in-container shell and the `docker run ... addressr-loader` command form are deliberately given up. Amended 2026-07-26 by ADR-040: tags become `:<version>-<gitsha>` plus `:latest` with bare semver release-only, publishing moves into CI behind a reusable workflow, and the digest-pin trigger fired, was assessed, and the pin was declined — a float that re-resolves on every rebuild beats a pin with no refresh mechanism.
**Confirmation:** `.github/workflows/docker-image.yml` job `build-and-smoke`, none of it run yet: `npm run build:docker` completes; `docker inspect` reports a non-root user; the container answers a real HTTP request on `/health` (the check that catches an unresolvable `CMD` path); `docker stop` terminates in under 10s without `dumb-init`; manually verify the loader writes `/home/nonroot/target/keyv-file.msgpack` without EACCES. These criteria transfer unchanged to ADR-040's reusable `workflow_call` definition when stage 2 creates it; both locations are named so they stay checkable in the meantime.
