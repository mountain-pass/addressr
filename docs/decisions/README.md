...
**Decides:** Publish `mountainpass/addressr` as a two-stage build (`node:22-bookworm-slim` builds, `gcr.io/distroless/nodejs22-debian12:nonroot` runs) so the runtime carries only node, the app's dependency tree, and a vendored `tini` — no shell, package manager, or npm — with uid 65532, a writable `/home/nonroot` WORKDIR, a resolved `CMD` script path, and re-declared `ELASTIC_*`/`ADDRESSR_INDEX_*` defaults preserving the runtime contract, at the cost of the in-container shell and the `docker run ... addressr-loader` command form. Amended 2026-07-26/27: `tini` runs as PID 1 after CI disproved the claim that node handles `SIGTERM` as PID 1 (ADR 013's signal driver was right; graceful shutdown deferred to P067; bookworm/glibc stage parity is now load-bearing), and ADR 040 moved tagging to `:<version>-<gitsha>` plus `:latest` with bare semver release-only, moved publishing into `docker-image.yml` itself via `workflow_call` with a `publish_semver` input (not a separate reusable workflow, and the PR guard is an inline `if:` rather than a caller-passed input), and discharged the digest-pin trigger — a float that re-resolves every rebuild beats a pin with no refresh mechanism.
