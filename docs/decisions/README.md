...
**Decides:** Splits the release pipeline into three independently-triggered axes — npm publish on a consumed changeset, docker publish on a release or a `Dockerfile`/`.dockerignore.tmpl`/docker-workflow-scoped master push, and deploy on a release, a `deploy_only` dispatch, or a `deploy/**` push excluding `deploy/.terraform.lock.hcl` — so no axis is reachable only by laundering an unrelated version bump. Adds immutable `:<version>-<gitsha>` tags so a rebuild can never re-point a consumer's pin, and moves publishing into CI; amended 2026-07-28 to publish to `ghcr.io/mountain-pass/addressr` under `GITHUB_TOKEN` instead of Docker Hub, which discharges the registry-replacement reassessment trigger.
**Amended 2026-07-26 (stage 2), 2026-07-27 (stage 3), 2026-07-28 (GHCR), as built:** `docker-image.yml` keeps its push-to-master trigger (input is `publish_semver`, not `push`) with build/smoke/publish in one job so the pushed image is the smoke-tested image per ADR-039; the stage-2 double-publish blocker is RESOLVED via option C — `package.json`/`package-lock.json` dropped from the push path filter, so a changesets commit no longer self-fires and the guard conjunct (with its empty-string coercion trap) is deleted; `release.yml` went multi-job and `release-watch.sh` was rewritten to match, closing a P004-class false green; the GHCR move retires guard 2 (`GITHUB_TOKEN` is always present; fork tokens are read-only) leaving guard 1 load-bearing, and break-glass now needs a `write:packages` PAT; JTBD-400 is factually contradicted, not merely stale; all stages landed ahead of the ratification drain the Confirmation criteria required.
