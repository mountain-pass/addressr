...
- **2026-07-24** — Additive cross-reference for [ADR 037](037-cors-preflight-caching-policy.proposed.md) (P023 fix): ADR 037 introduces an `OPTIONS` **method-level** exemption — CORS preflights are now answered `204` before this middleware runs — so the enforcement surface is narrowed on the method axis alongside the existing `/health` + `/api-docs` **path** allowlist. Enforcement of data-carrying methods is unchanged; `human-oversight: confirmed` preserved (this is an additive back-reference, not a decision reversal).
- **2026-07-25** — Correction to the 2026-07-24 entry, per ADR-037 remediation R1 (pre-push risk-scorer STOP 6/25 → within appetite). The `OPTIONS` method-level exemption is **not unconditional**: it applies **only when CORS is enabled** (`ADDRESSR_ACCESS_CONTROL_ALLOW_ORIGIN` set), because the preflight handler is now gated behind that opt-in. On a CORS-disabled origin `OPTIONS` falls through to this middleware (prior behaviour) — narrowing the exemption strictly tightens the enforcement surface, so this is more conservative than the 2026-07-24 shape. `human-oversight: confirmed` preserved (additive corrective maintenance; the core proxy-auth decision is unchanged).
