...
- **RESOLVED 2026-09-05: the chosen terminus exists and is buildable, with one requirement still to confirm empirically.** The bullet below is retained as written, because it was the state of knowledge when this decision was made and the reassessment trigger it carries is what prompted the check. What the check found: a Worker's send binding can send to a **verified destination address** with only Email Routing configured — the provider's documentation states that sending to verified destination addresses "is free on all plans, even when only Email Routing is configured" — so no separate sending onboarding, no messaging vendor and no stored credential is needed. The installed Terraform provider (5.24.0) carries four resources — `email_routing_settings`, `email_routing_address`, `email_routing_rule`, `email_routing_catch_all` — and an `email_routing_dns` data source, so the whole configuration is declarable; only the destination address's verification needs a human, which is the same shape as the existing operations-topic email subscription. Enabling it replaces the domain's inbound mail records, which point at the registrar's default forwarding. The maintainer confirmed on 2026-09-05 that no address at the domain reaches them — the zone carries MX records, so it is configured to accept mail, but nothing arrives anywhere. On that basis those records are cruft and replacing them displaces nothing; note this is a claim about delivery, not about the zone never having accepted mail. The credential-free ground the chosen option won on therefore holds. **One requirement is not settled:** the sender address a Worker must use when only Email Routing is configured is NOT stated in the provider's documentation. That is recorded as needing empirical confirmation rather than assumed, and it can only be confirmed once routing is in place.
