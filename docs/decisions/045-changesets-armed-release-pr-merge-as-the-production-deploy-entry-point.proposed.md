...
1. **Changesets-armed release-PR merge (chosen)** — an infra change carries a changeset for `apps/addressr-deployment`; the deploy gate fires on that package's version bump, and merging the release PR is the apply.
...
1. `apps/addressr-deployment/package.json`'s `version` **VALUE** differs between `github.event.before` and HEAD. Read both ends explicitly — `git show <BEFORE>:apps/addressr-deployment/package.json` — and compare the parsed values. This is **not** a path diff. `git diff --name-only` on that path presents a future move of `apps/addressr-deployment/` as a change to the file and would arm a production apply on a rename, which is the exact defect that retired the predecessor axis. A value comparison is structurally immune: on a rename the HEAD-side read _fails_ rather than _differs_.
...
- The decision to deploy becomes a committed artefact — reviewable in the release PR, recorded in `apps/addressr-deployment/CHANGELOG.md`, and attributable — where the dispatch recorded nothing.
...
2. **Value-not-path proved.** A test asserts the detection step reads `git show <ref>:apps/addressr-deployment/package.json` and compares parsed values, and that the literal step id `deploy-paths` appears nowhere in `release.yml` including comments.
...
5. **Changeset guard. RULE SETTLED 2026-08-10, later the same day.** A change to `apps/addressr-deployment` infrastructure that will arm no deployment version bump is caught before it can sit unapplied. Runs as a `pre-push` hook **and** as a CI leg over `github.event.before..GITHUB_SHA` — on the grounds that a client-side-only gate is bypassable and the failure it guards is silent and green.

   **The rule: BLOCK when nothing pending will bump `apps/addressr-deployment`; PASS when an app changeset will cascade.** Blocking the cascade case would be a false red on the ordinary mixed push, where the apply genuinely will happen.
