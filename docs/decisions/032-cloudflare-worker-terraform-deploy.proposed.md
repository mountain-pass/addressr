...
ADR 018 Reassessment Criterion at line 63 explicitly names "Version-controlling the worker script" as the trigger for revisiting that posture. [P040](../problems/closed/040-uptime-robot-401-api-addressr-missing-proxy-auth.md) was the realised cost: a latent `safeIps.includes(srcIp)` bug rejected IPs inside CIDR ranges Uptime Robot publishes, and the worker source not being in the repo meant the fix could not be unit-tested or PR-reviewed — only pasted-from-chat into the dashboard. [P042](../problems/closed/042-version-control-cloudflare-worker-via-terraform.md) is the ticket this ADR exists to close.
...
- Consistency with the rest of `deploy/` — the AWS path is Terraform-managed via the existing 1Password Voder → GitHub Actions → Terraform secret flow ([ADR 030](030-opensearch-domain-terraform-module.accepted.md) Scope; `reference_addressr_secrets` memory); Cloudflare should not fork the secret spine.
...
- [ADR 030 — OpenSearch Domain Terraform Module](030-opensearch-domain-terraform-module.accepted.md) — module-shape and Confirmation-language precedent for this ADR. The asymmetry (sibling source directory `deploy/cloudflare-worker/`) is the only structural difference.
- [Problem P040 — Uptime Robot 401 alerts: Cloudflare Worker safeIps CIDR-match bug + UR IP drift](../problems/closed/040-uptime-robot-401-api-addressr-missing-proxy-auth.md) — the realised cost that motivated bringing this work to the front of the queue.
- [Problem P042 — Version-control the Cloudflare Worker via Terraform](../problems/closed/042-version-control-cloudflare-worker-via-terraform.md) — the ticket this ADR closes.
