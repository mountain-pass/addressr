...
- **This criterion is NOT HELD by an executing instrument.** The unit test at `service/address-service.test.js` does assert that `mapAddressDetails` populates `ssla === sla` for street-level addresses and a distinct short form for sub-unit addresses — but no runner globs `service/*.test.js`. `test:js` is `node --test test/js/__tests__/*.test.mjs`, and nothing in `package.json` or `.github/workflows/` reaches outside it, so those assertions have never executed. Recorded 2026-08-09 rather than left implicit, because the compendium renders this bullet first and it read as satisfied. The BEHAVIOUR is still covered — by the un-skipped P007 Cucumber scenario below and by ADR 027's pins in `test/js/__tests__/address-service.test.mjs`, both of which do run — so the decision is not unpinned; the named instrument is. Tracked as one of five orphaned test files.
