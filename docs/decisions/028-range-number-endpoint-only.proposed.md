...
- **`sla_range_expanded` DEPRECATED 2026-08-09, and its index-side mechanism has never executed** (P091 — written to `structured.sla_range_expanded`, a path the mapping never indexed, on 0 of 16,905,824 documents). ADR 028 itself is unchanged and still `proposed`; only the field is deprecated. The **attachment** is retained for consumer/ETag compatibility only, and is now covered by executing assertions in `test/js/__tests__/build-indexed-document.test.mjs` against the extracted `src/build-indexed-document.js` — which deliberately CHARACTERISE today's wrong placement rather than confirming that index-side Option A works. The **query-side clause-wiring** assertions have been re-pointed twice, both times **re-pointed, not deleted**, so Reassessment Criterion 5 has never fired.
  - _Amended 2026-08-09 (instrument, index side)_: the clause "the `sla_range_expanded` attachment tests (`mapAddressDetails`) are untouched, remain source-pattern, and remain valid — index-side Option A is retained in full" was true when written and is not now. P033's first conversion moved all three assertions out of `test/js/__tests__/address-service.test.mjs` into `build-indexed-document.test.mjs` on 2026-08-08 — the third instance of the same re-point-not-delete pattern the query-side amendments below record twice. It was left behind because both prior amendments addressed the query side only, which is ADR 031's append-elsewhere failure. This bullet was also moved to first position: ADR 028 has no `Chosen:` line, so its Confirmation bullets are its entire compendium surface, and at its former position a reader met two "the mechanism works" assertions before reaching the fact that it has never run.
...
**Amendment, 2026-08-09 — the retention rationale below is WITHDRAWN. The field is deprecated.**

Everything under the heading **"SUPERSEDED 2026-08-08 retention rationale"** below is withdrawn — named rather than pointed at, because a positional reference ("the two paragraphs after this one") was what the first draft of this amendment used, and it pointed at the new evidence rather than the old rationale. The withdrawn text argues the aliases are retained because they are _"what the follow-up anchors on"_ — the follow-up being a `.raw` subfield plus a `dis_max` entry that would restore this decision's endpoint-ranking win. That argument no longer stands, because the win it anchors on is not wanted.

Measured against production on 2026-08-09, 500 range addresses on a fresh seed (`test/perf/range-alias-value-probe.mjs`, recorded on P091): in the only case where the alias could help — neither endpoint exists as its own address, 38.6% of range addresses — the range document was found in page **386 of 386** times **without** the alias, 368 of them at rank #1. In the remaining 61.4%, the alias would boost a range document against the actual address at that number, which is a street-level-first violation rather than a recall win. Two independent biases in the measurement both understate the harm side.

So the endpoint-ranking win is not a suspended asset. Restoring it would make ranking worse.

**What retention now rests on, and it is the only thing.** The aliases stay in the index and in the response for **consumer compatibility**: removing the field changes the `GET /addresses/{id}` response body and therefore the ETag for 349,540 range addresses. That is a liability being wound down on a notice period, not an asset being held. Both specs now mark the field deprecated — the OpenAPI 3 document served at `/api-docs` sets `deprecated: true`, and the shipped Swagger 2.0 file carries the notice in its description, that dialect having no such keyword.

**Reassessment trigger for the removal:** the next full load. P091 records that the `_source` change rides a load boundary naturally, so removal costs nothing extra when taken there. Until then the field stays, non-functional and announced.

### SUPERSEDED 2026-08-08 retention rationale — withdrawn 2026-08-09, retained for audit

Everything from here to the heading **"Still live"** below is superseded. Deleting it would destroy the record that this decision changed on evidence; leaving it unbounded would make the live text after it read as history, which is the defect this heading pair exists to prevent.
...
**Released as a MINOR, and the reasoning is recorded because this ADR sets the precedent it sits against.** This ADR's own line reads that "any consumer-visible ranking or result-set change on the RapidAPI listing warrants a minor bump" — scoped to BEHAVIOUR, and the deprecation changes no response, so patch would not have contradicted it. Two things decide it the other way. Adding optional properties to a published response schema is an additive interface change, which is a minor under semver; only the `geo` and `ssla` corrections are fixes. And the change exists so that a future removal is announced rather than silent — shipping that announcement as a patch puts it on the quietest channel available, which works against its own purpose. A deprecation that consumers pull without reading is not a deprecation.
...
**ADR 027 interaction.** [ADR 027 — `fuzziness: 'AUTO:5,8'`](027-fuzziness-auto-5-8.proposed.md) Reassessment Criterion 3 names "`sla_range_expanded` symmetric population per ADR 026 Option C" as one of its two prescribed remedies should a new ranking inversion emerge. That remedy is **DEAD as of 2026-08-09, not merely unreachable** — amended here in the same change that deprecated the field. The earlier wording said it required re-introducing a query-side carrier and offered "or re-open this amendment first" as a route. That route is closed: re-introducing the carrier would make ranking WORSE, measured at n=500 against production, and ADR 027's Criterion 3 now says so in the same terms. **The numeric-exactness filter is the only live remedy.** Both files were corrected in one commit precisely so they cannot disagree about it.
