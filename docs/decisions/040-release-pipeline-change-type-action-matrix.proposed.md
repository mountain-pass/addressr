...
> **SUPERSEDED by [ADR 050](050-the-image-follows-the-publish-not-the-deploy.proposed.md).** This describes the re-credentialed shape of a break-glass that is no longer sanctioned. Recorded because amendments outrank body text here, so retiring the body-text site alone would leave this reading as live.
...
> **SUPERSEDED by [ADR 050](050-the-image-follows-the-publish-not-the-deploy.proposed.md).** The scripts remain and the pipeline still invokes `docker:push` — what is withdrawn is their standing as a sanctioned MANUAL route. Registry pushes come from the pipeline (user direction 2026-08-19).
...
> **PARTIALLY SUPERSEDED by [ADR 050](050-the-image-follows-the-publish-not-the-deploy.proposed.md).** "A failing `release` job skips the docker publish" is no longer true on the publish path, and "not publishing from a red tree is desirable" is the judgement ADR-050 reverses. The P044 case still skips — but because P044 fires only when `published != 'true'` while the gate requires `== 'true'`, not because of `needs: release`.
...
> **SUPERSEDED by [ADR 050](050-the-image-follows-the-publish-not-the-deploy.proposed.md).** The docker axis is no longer coupled to the deploy axis — `docker-publish` now carries `!cancelled()`, so a post-publish failure cannot skip the image. The "benign" ground below is also withdrawn: the manual `docker:push` it degrades to is no longer a sanctioned route. Realised in production on 2026-08-19 releasing 3.3.2.
