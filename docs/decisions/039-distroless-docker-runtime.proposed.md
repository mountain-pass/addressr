...
> **Oversight note.** The substance of this decision — Distroless over Alpine, shell-loss accepted — was taken by the user on 2026-07-18 during the `/wr-architect:review-decisions` drain and is recorded on [P055](../problems/known-error/055-migrate-docker-image-alpine-to-distroless.md). This ADR was authored by an AFK iteration with no interactive access, so it was left for the `/wr-architect:review-decisions` drain to ratify rather than self-certifying against that provenance. The frontmatter carries the ratification state and this sentence deliberately does not restate it: until 2026-09-07 it said the record "carries" a marker value it had not carried since July, which is what a second copy of a fact does. [ADR 040](040-release-pipeline-change-type-action-matrix.proposed.md) is in the same state and amends this ADR in three places; **ratify both in the same drain pass**, and do not let ADR 040 reach `accepted` first.
>
...
> **SUPERSEDED IN PART by [ADR 050](050-the-image-follows-the-publish-not-the-deploy.superseded.md).** The break-glass `npm run docker:push` route named here is withdrawn as a sanctioned manual path.
...
> **SUPERSEDED IN PART by [ADR 050](050-the-image-follows-the-publish-not-the-deploy.superseded.md).** "The manual scripts survive as documented break-glass" no longer holds: registry pushes come from the pipeline (user direction 2026-08-19). The scripts remain and CI still invokes `docker:push`; what is withdrawn is their standing as a sanctioned MANUAL route. Note the irony this record should keep — a manual push skips this ADR's own smoke steps, voiding the compensating control the floating `:nonroot` base was accepted on.
