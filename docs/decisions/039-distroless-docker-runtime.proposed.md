...
This is `tini` now **plus** the graceful-shutdown handler later, not `tini` instead of it. Option 1 remains wanted for a different property — draining in-flight requests — and is tracked as [P067](../problems/open/067-no-sigterm-graceful-shutdown-handler.md). It is deferred, not dropped.
...
- In-flight request loss at shutdown becomes concrete for a self-hoster (dropped connections on restart or redeploy) — that is [P067](../problems/open/067-no-sigterm-graceful-shutdown-handler.md), the graceful-shutdown handler, not another init change
