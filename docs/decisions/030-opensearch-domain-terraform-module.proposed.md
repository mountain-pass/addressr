...
- **Credential wiring is 1P → GHA secrets → `sync-tfc-vars.yml` → TFC workspace variables → Terraform (amended 2026-07-07).** The workspace uses remote execution (`backend "remote"`), so TFC workspace variables — not the `TF_VAR_*` env vars release.yml exports — are what terraform runs consume. The manual GHA→TFC copy leg was the P028 failure surface; `.github/workflows/sync-tfc-vars.yml` (workflow*dispatch) mechanises it: upserts `elastic_v2_username`/`elastic_v2_password` as sensitive terraform-category variables from the GHA secrets and deletes stale `elastic_v2*\*`overrides. GHA secrets are the effective source of truth for the v2 cred plane; TFC is mechanically downstream.`TERRAFORM_CLOUD_TOKEN`'s role expands from backend auth to TFC Variables-API writes. Sensitive TFC variables are write-only, so verification is write-path success (non-2xx fails the job), never read-back.
- **Audit-log publishing (added 2026-07-06, P036) — RETIRED 2026-07-07 by ADR 033.** Originally the module provisioned a per-domain CloudWatch audit-log group + resource policy + `AUDIT_LOGS` publishing to trace the FGAC clobber. ADR 033 disables FGAC entirely (IAM/SigV4 auth), and AWS couples `AUDIT_LOGS` to FGAC — so the audit resources are removed with it. With no FGAC there is no `.opendistro_security` and no clobber to trace, so the rationale is moot. The P035 index-deletion trip-wire moves to a `SearchableDocuments`-drop CloudWatch alarm in the caller (ADR 033, human-confirmed 2026-07-07). The `enable_audit_logs` / `audit_log_retention_days` module variables are removed.
...
- Before 2026-09-30 (aligned with frontmatter `reassessment-date`, bumped 2026-07-06 with the ADR 029 re-attempt), whichever comes first.
