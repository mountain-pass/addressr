...
**Risk-driven refinement (2026-07-25, R1).** As originally committed the handler was _always on_ (defaults `|| '86400'` / `|| 'GET,OPTIONS'`), registered unconditionally. The pre-push pipeline risk-scorer returned STOP at 6/25 (one above the 5/25 appetite) on that defaults-on-everywhere blast radius. Remediation R1 (**wr-risk-scorer ADR-042 Rule 1** — auto-apply scorer remediations incrementally until residual is within appetite, halting per that ADR's Rule 5 if it cannot converge; plugin-scoped, **not a record in this repo's `docs/decisions/`**, whose 042 slot is now OCCUPIED by an unrelated intra-repo decision. This repo's ADR-042 is Anchored span phrase clause for street-level-first ranking, a search-relevance decision with no connection to risk scoring. The collision is live rather than merely possible as of 2026-08-07, so the bold plugin-name-first citation form above is load-bearing here, not stylistic — applied in the same commit) **gates the whole `app.options` registration behind the same `ADDRESSR_ACCESS_CONTROL_ALLOW_ORIGIN !== undefined` opt-in the sibling CORS response headers use**. `Access-Control-Max-Age` is inert without `Access-Control-Allow-Origin`, so this loses no efficacy: when CORS is enabled the fix applies exactly as user-approved (Max-Age `86400` default, Allow-Methods `GET,OPTIONS` default, 204); when CORS is not enabled the handler is not registered, no cache directive is emitted, and the OPTIONS auth-exemption does not exist. This is the shape the user ratifies at the `/wr-architect:review-decisions` drain — not the stale always-on text.
...
**Ordering invariant** (regression-guarded): `app.options(...)` MUST precede `app.use(proxyAuthMiddleware())`. A reorder would silently make preflights 401 (or bypass a future method-sensitive auth check). Guarded at two tiers, both behavioural since 2026-08-09: `test/js/__tests__/waycharter-server.test.mjs` drives `buildRest2App` through `light-my-request` with no search backend, so it runs in pre-commit and on the engine floor; `test/resources/features/cors-preflight.feature` covers the same properties over live HTTP in the embedded profile. Both pin OPTIONS→204 AND GET-without-secret→401 under proxy auth. The first was a source-inspection test until ADR-044 made the module importable.
...
- **Gating pinned behaviourally (R1):** with `ADDRESSR_ACCESS_CONTROL_ALLOW_ORIGIN` unset and proxy auth enabled, `OPTIONS /addresses` is **not** 204 and carries no `Access-Control-Max-Age` — the handler is not registered, so the preflight is subject to authentication like any other method (`waycharter-server.test.mjs`, executing via `light-my-request`). **Amended 2026-08-09.** This criterion previously cited a source-inspection instrument in `waycharter-server.test.mjs` that did not exist: none of that file's assertions checked the gating. So the criterion was unmet and read as met, which is the same defect the 2026-08-03 correction in that file was written about, one criterion over. Mutation-proved by removing the `isPreflightEnabled()` guard, which fails this case alone.
- **Exemption stays OPTIONS-scoped:** the only pre-`proxyAuthMiddleware` method short-circuit in `buildRest2App` is `app.options` — no data-carrying method (`all`/`get`/`post`/`put`/`delete`/`patch`) is answered ahead of enforcement (`proxy-auth.test.mjs`, source-inspection), backed by the runtime `GET /addresses` → 401 coverage in the same file.
- **Ordering invariant:** with CORS and proxy auth both enabled, an unauthenticated `OPTIONS /addresses` is 204 with `Access-Control-Max-Age: 86400` **while an unauthenticated `GET` on the same path is 401** (`waycharter-server.test.mjs`, executing via `light-my-request`). **Amended 2026-08-09** from a source-inspection instrument comparing statement offsets. The 401 is part of the criterion, not context: without it the 204 is satisfied by authentication being disabled, and the check passes through the regression it names. Mutation-proved by moving `app.options` after `proxyAuthMiddleware`, which fails this case alone.
