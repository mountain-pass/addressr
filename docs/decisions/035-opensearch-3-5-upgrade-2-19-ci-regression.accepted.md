...
5. **v2 decommissioned post-soak** — `addressr4` (OpenSearch 2.19) destroyed 2026-07-14 via `terraform apply` (module + IAM policy + OIDC loader role + drop-alarm + vars removed; 9 loader crons retargeted to `target=v3`; parity dashboard → v3-only). `addressr5` is the sole remaining domain. **Superseded 2026-08-02:** ADR-041's analyzer migration provisioned `addressr6`, which is now the sole domain — `addressr5` was decommissioned 2026-08-02 (commit `2e557b9`) once the P079 retention gate was met. This bullet remains as the dated record of the 2026-07-14 v2 teardown; it is history, not current state. Rollback is now rebuild-from-G-NAF, the accepted Option C trade. ✅
