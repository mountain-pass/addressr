...
- **Good**: 1-character diff. No query shape change. No mapping / reindex / storage impact. ADR 025 summation-symmetry preserved (field list, clause structure unchanged). ADR 028 `phrase_prefix` clause (inherited from the superseded ADR 026), `sla_range_expanded` field, and `tie_breaker=0.0` invariant all untouched. Rollback is trivial (revert the 4-char string).
  > **Correction, 2026-08-09 — the recall guarantee this rejection protects never operated.** Two independent facts falsify the premise. ADR 043 removed the `phrase_prefix` clause on 2026-08-08, so there is no clause to be "left in". And P091 established that `sla_range_expanded` was populated on **0 of 16,905,824** production documents — written to a path the mapping does not cover — so the field carried nothing to regress. ADR 028 has since deprecated it outright on a measured finding that restoring it would make ranking worse. **Option D's rejection stands on its other grounds**; this particular ground is withdrawn. Enumerated rather than patched inline because this claim is restated at more than one site in this file, and correcting one restatement while its siblings stay live is the failure this correction is an instance of.
...
**Interaction with ADR 028 — explicit non-triggering**: ADR 028 (which supersedes ADR 026 in the same v2.4.0 release) inherits the same `phrase_prefix` / `sla_range_expanded` / `tie_breaker=0.0` invariants. This change does not add `sla_range_expanded` to `bool_prefix` (**corrected 2026-08-09: it was confined to the `phrase_prefix` clause, which ADR 043 removed on 2026-08-08 — so it is confined to nothing, and ADR 028 has since deprecated the field entirely**). It does not touch `tie_breaker`. ADR 028 invariants hold.
...
- **Unit test** in `test/js/__tests__/address-service.test.mjs`: behavioural assertion that the `bool_prefix` `multi_match` clause returned by `buildAddressSearchBody` (`src/build-search-body.js`) sets `fuzziness: 'AUTO:5,8'` (not `'AUTO'`, not `'AUTO:6,8'`, not `'AUTO:4,7'`). Amended 2026-08-07: was a source-pattern regex over `searchForAddress` in `service/address-service.js`; the query body was extracted to clean ESM so the assertion could be made against the built object rather than the source text (P033, and P074 Fix Strategy prerequisite 3 — the integration test was hand-copying a body that had already diverged). The invariant is unchanged; only the instrument is.
...
- `AUTO:5,8` proves insufficient and a new ranking inversion emerges on query shapes not covered by the baseline. At that point, reconsider Option D (query token-type split) with the architect-recommended shape (filter for numeric exactness, or `sla_range_expanded` symmetric population per ADR 026 Option C **(DEAD, not merely unreachable — amended 2026-08-09. [ADR 043](043-keyword-prefix-anchor-for-street-level-first-ranking.accepted.md) removed the only clause targeting that field, and [ADR 028](028-range-number-endpoint-only.proposed.md) has since DEPRECATED it: measured against production at n=500, restoring it would make ranking worse, not better. Do not route a future reader here. The numeric-exactness filter is the only live remedy.)**).
