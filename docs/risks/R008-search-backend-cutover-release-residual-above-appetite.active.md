...
- 2026-08-10 (third entry today): **Re-verified after R020 was re-scored against the changesets-armed successor route**, which is now BUILT and is the sole path to a production infrastructure apply — `deploy_only` is deleted. **Direction: unchanged in score, worse in one dimension worth naming.** The cutover residual is a property of the cutover. But a cutover ROLLBACK is itself a Terraform apply, and it must now travel the changesets route — changeset, release PR, CI, merge — rather than a single dispatch. ADR-029's "rollback within 10 minutes, discharged at 6m36s" was measured against the dispatch and does not transfer. Recovery is slower than when this entry was last scored. Priced on R020's re-score rather than re-rated here, because this entry owns the cutover, not the route. No re-rate.

- 2026-08-10 (second entry today): **Re-verified after [R020](R020-deploy-path-push-tier-prod-deploy-precondition-unmet.active.md)'s treatment and monitoring were re-pointed** off the `deploy_only` dispatch and onto the changeset-armed release-PR route, per the user-ratified JTBD-400 amendment in commit `09f6418`. **Direction: unchanged.** The cutover residual is a property of the cutover, not of the entry point that initiates it — the same reasoning as the entry below, and it survives the entry point changing identity rather than merely narrowing. No re-rate.

- 2026-08-10: **Revisited for the `deploy/**` push-axis retirement.** [R021](R021-push-tier-deploy-axis-arms-prod-terraform-apply.retired.md) and [R022](R022-unstaged-terraform-lockfile-drift-arms-deploy-axis.retired.md) retired (hazard deleted, not reduced); [R020](R020-deploy-path-push-tier-prod-deploy-precondition-unmet.active.md) re-scored 8 → 10 because retiring the axis deleted the ground its Impact 4 rested on. Governance: [ADR 001](../decisions/001-risk-gated-release-process.proposed.md) and [ADR 040](../decisions/040-release-pipeline-change-type-action-matrix.proposed.md), 2026-08-10 amendments. **Direction: unchanged.** The cutover residual is a property of the cutover, not of the entry point that initiates it. One second-order note: a cutover now has one fewer way to be started accidentally, which is favourable and too small to move a score. No re-rate.
