...
There is no way to publish a package version without also deploying to production. `release.yml`'s **Deploy new version** step is gated on `success() && (steps.changesets.outputs.published == 'true' || steps.deployment-version.outputs.changed == 'true')` — quoted in full because both the `success()` and the PARENTHESES are load-bearing (`&&` binds tighter than `||`, so `success() && A || B` would deploy after an upstream failure), and because this is the one string `release-workflow-deploy-only.test.mjs` pins character-exactly as `DEPLOY_GATE`. **Corrected twice on 2026-08-10.** It first quoted a third disjunct, `steps.deploy-paths.outputs.changed == 'true'`, which retired with the `deploy/**` push axis. It then quoted `inputs.deploy_only == true` as the second disjunct; that input was DELETED at `8199e5b9` and the changesets-armed deployment version bump replaced it (ADR-045). **Line numbers are deliberately dropped from this paragraph** — they were cited as `:348` and then `:358`, and both rotted within a day. Locate the gate with `grep -n "Deploy new version" .github/workflows/release.yml`. The correction narrows the quoted gate; it does not weaken this entry, because the retired disjunct was the one path that deployed WITHOUT publishing and so was never part of the coupling this entry names. The first disjunct is the coupling: a changeset merging to master publishes to npm and, in the same job, deploys to Elastic Beanstalk and smoke-tests production.

**The coupling is a decision, not an oversight, and should not be "fixed".** ADR-001's 2026-07-26 amendment states the asymmetry in terms — every publish is followed by a deploy, not every deploy needs a publish — and ADR-040 encodes `published == 'true'` as the first disjunct of the deploy row in a change-type-to-action matrix whose entire purpose was decoupling. It survived that decoupling deliberately. P039 gives the mechanical reason: the manifest `deploy.sh` generates at `apps/addressr-deployment/deployment/package.json` pins `"@mountainpass/addressr"` at the version `apps/addressr-deployment/deploy.sh` resolves, so **the EB instance installs the running artefact from the public npm registry**. _Mechanism corrected 2026-08-08: that version used to come from `npm_package_version`, the job's working tree, which is what P095 broke on; `deploy.sh` now resolves it once via `./resolve-version.sh` and interpolates `${deploy_version}` at every site. **The conclusion is unaffected** — the manifest still pins from the public registry, so `publish ⇒ deploy must stay` holds and this entry's residual does not move._ A publish not followed by a deploy leaves production pointing at a version behind the registry. P039 records `publish ⇒ deploy must stay` as a constraint on any fix.
...
- **EB health-gated rolling deploy with `RollbackLaunchOnFailure`** — implemented in `apps/addressr-deployment/main.tf`. Evidenced: exercised and timed at 6m36s push-to-EB-updated during the ADR-041 rollback drill (commits `43b3309`, `f295bd8`). Note the caveat recorded on R003: `BatchSize = 100 Percentage` cycles the whole fleet, so "Rolling" does not mean a partial-fleet blast radius.
- **`Wait for deployment to stabilize` then `Smoke test production`** — gated on the same condition as the deploy, so a release cannot report green without production answering. _(Line numbers dropped 2026-08-10: cited as `:404,408`, since moved. The step NAMES are the anchor, and `release-workflow-deploy-only.test.mjs` asserts both of them by name — added in the same batch as this citation, because until then they were reachable only through a name-blind count and this anchor would have rotted SILENTLY, which is worse than the line numbers it replaced.)_
...
- **P044 swallowed-publish assertion** — the `Fail if a publish was expected but did not happen (P044)` step fails the run when a publish was expected but did not happen, making a broken publish→deploy loop loud rather than silently green. _(Line number dropped 2026-08-10; also note it NARROWED back to two conjuncts when `deploy_only` was deleted, so it now runs on a deployment-only release — and passes, because it reads `packages/addressr/package.json`, which a deployment-only changeset does not bump.)_
...
- **Likelihood**: 2 (Unlikely) — the smoke gate plus health-gated rollback means a prod-breaking change is caught and reverted rather than left serving. **Which rollback, stated 2026-08-10 because it now matters:** this is the EB-level `RollbackLaunchOnFailure` on the application version, which ADR-045 does not touch. It is NOT the Terraform-level rollback, which got slower — R020 records two serialised CI runs ahead of an infra apply, and ADR-029's measured 6m36s does not transfer to the changesets-armed route. This entry's 2 does not depend on that path; a future reader should not credit it to one that has become slower.
- **Residual Score**: 8
...
The production smoke assertion is **hard-coded**, not parameterised: the `Smoke test production` step asserts `/debug/shadow-config` reports `hostSet` **is** `false` — the read-shadow-DISABLED posture, failing the run on anything else. _(Polarity corrected 2026-08-10: this entry previously said it asserts `hostSet != false`, which is backwards. The workflow carries a `READ THE POLARITY BEFORE EDITING` block above that assertion precisely because inverting it is how P035 ran green for eight days, and the correct posture flips with whether a migration is in flight. The argument below holds either way, but the quoted condition should not be the one thing a reader can misread.)_ _(Line number dropped 2026-08-10; grep `hostSet`.)_ So an env-driven shadow-config flip applies Terraform _first_ and only then fails its own smoke check — leaving production flipped behind a red run, with the deploy already applied and no automatic rollback for the config change. This is P039's open smoke-parameterisation task, and it is the sharpest edge on this entry: the control that makes the residual tolerable is the same control that fails open for one specific change class.
