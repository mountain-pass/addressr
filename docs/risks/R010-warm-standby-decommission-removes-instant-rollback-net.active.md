...
- **EB health-gated rolling deploy.** `RollingUpdateType: Health`, `MinInstancesInService: 2`, `RollbackLaunchOnFailure: true`, with `/health` probing the search backend. Bounds a bad flip to a degraded subset rather than a fleet-wide outage. In `deploy/main.tf`.
