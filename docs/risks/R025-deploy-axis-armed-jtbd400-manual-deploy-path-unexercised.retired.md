...
Scoring lives on R020. **As at this retirement (2026-08-04)** it carried a measured split: the axis half discharged by three successful production applies, each verified by reading the `Deploy new version` step's conclusion, while the recovery half was not — all four `workflow_dispatch` runs to that date had skipped every deploy step, so `deploy_only=true` had never been dispatched. R020 held the residual at 8/25, above appetite, with a one-action treatment: dispatch it once.

**Superseded 2026-08-05, and left dated rather than rewritten.** That treatment was carried out — two `deploy_only=true` dispatches, both running the deploy steps rather than skipping them — so the "never been dispatched" clause above is true only of the period this entry describes. Its residual moved for a different reason than this entry records — the exercises ran against a plan that changed nothing. **Read R020 for current state; this entry is a snapshot.** Dating the claims rather than maintaining them is the discipline for a retired record — a dated claim stays true, a bare one rots.
