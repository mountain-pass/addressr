...
- 2026-08-23: **Re-verified after P039 closed and R015's link to it was repointed.** **Direction: unchanged, and the close CONFIRMS this entry's half of the split rather than disturbing it.** R015's only edit was the moved-path repoint (`known-error/` → `closed/`) that ADR-048 obliges. P039 asked whether a deploy can happen without a publish; ADR-045 answered yes, via a changeset on the `private: true` deployment package. That is exactly the publish-free route this entry owns, and the close is evidence it exists rather than a change to it. R015 still owns the other direction — a publish always deploys — which the close does not touch. The deliberate separation recorded on 2026-08-10 holds, and is now better evidenced. No re-rate.
