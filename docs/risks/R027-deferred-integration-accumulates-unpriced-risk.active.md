...
- 2026-08-05: Cross-reference to R022 re-verified after R022's Description was re-scoped from the provider-lockfile instance to the `deploy/**` working-tree class in the same sitting. The citation here is about R022's _title-versus-class_ rescue, not about its Description's scope, so it still holds unchanged. Recorded rather than left implicit because the review-fence check flags any entry that refers to one which has since moved — this is that check's first real use, and the answer it wanted was a verification, not a touch.
