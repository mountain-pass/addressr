...
- 2026-08-10 (third entry today): **Re-verified after R020 was re-scored against the changesets-armed successor route**, which is now BUILT and is the sole path to a production infrastructure apply — `deploy_only` is deleted. **Direction: the interim this entry watched is CLOSED, and that is the headline.** The prior note recorded a fully-specified, unbuilt successor as this entry's live instance — "the gap between record and artefact is now wide enough to be forgotten". The artefact landed: detector, gate at three sites, `deploy_only` deleted, 17 behavioural cases. The specification-ahead-of-code interim it named is over.

  **It closed the way this entry predicts such gaps close badly, which is worth recording as evidence rather than as a win.** The first implementation contained a defect the specification did not: the detector read a `HEAD` that `changesets/action` moves onto the version branch, so it would have armed an unreviewed production apply on the second changeset-bearing push of any release cycle. It passed the full suite. It was caught by risk scoring, not by the tests written from the spec — which is precisely this entry's thesis that a deferred integration accumulates risk the specification cannot see. No re-rate; the instance is discharged and the evidence strengthens the entry.

- 2026-08-10 (second entry today): **Re-verified after [R020](R020-deploy-path-push-tier-prod-deploy-precondition-unmet.active.md)'s treatment and monitoring were re-pointed** off the `deploy_only` dispatch and onto the changeset-armed release-PR route, per the user-ratified JTBD-400 amendment in commit `09f6418`. **Direction: the interim this entry watches just got LONGER and more specified at the same time.** The successor entry point is now described in detail across JTBD-400 and R020's Treatment — its trigger, its plan step, its sequencing — while none of it is built: `.github/workflows/release-pr-plan.yml` does not exist, and the deploy gate still reads `inputs.deploy_only`. A fully-specified, unbuilt successor is precisely this entry's subject, and the specification-ahead-of-code ordering is the favourable half (the constraint is reachable by whoever implements it). The unfavourable half is that the gap between record and artefact is now wide enough to be forgotten. Priced on R020 rather than left unpriced, which is what this entry asks for. No re-rate; this remains the live instance to watch.
