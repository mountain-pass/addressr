...
- 2026-08-09 (second entry today): Re-verified against R021's treatment ratification — the maintainer chose to harden the axis's per-disjunct preconditions rather than add a plan-approval gate or accept above appetite, and the second precondition (`source_hash` over the deployment bundle's manifest) landed with it. **This entry's citation holds**: this entry cites R021 as the axis's governance level in its Siblings clause, and R021's Treatment names this entry's subject as its operative off-path control, and none of that is reached by a treatment choice. Recorded because R021's Treatment and Controls both took body edits, so the fence correctly required its referrers in the same commit.
...
- 2026-08-09: Re-verified against R028's body change of the same date, which widened the review fence to walk committed history and date an entry at its last change outside its Change Log. **This entry's citation holds** — this entry cites R028 for the drift class its canonical-state declaration is built against, and widening a check's timestamp source touches no claim about drift or about this entry's subject.
...
- 2026-08-09: Re-verified against R021's re-rate (likelihood off Rare, residual 5 → 10 and above appetite, Treatment Accept → Mitigate) and R020's apply-count move to five. **This entry's citations still hold.** The Siblings clause describes R021 as "the axis's governance level" and R020 as the manual recovery path exercised 2026-08-05 against a no-op plan only; neither is a score claim, and the consolidation-rejected argument is about distinctness of hazard, which a re-score cannot reach. One thing is worth recording rather than left implicit: R021's Treatment still names _this_ entry's subject — keeping `deploy/**` out of unrelated commits — as the operative control, and that pointer is now load-bearing on an above-appetite entry rather than an at-the-line one. The soft joint this pair has always had did not change, but it now sits under more weight.
