...
- 2026-08-09: Re-verified against R008's 2026-08-09 bullet, which was itself a verification bullet confirming its citation of R028 holds. **This entry's Siblings distinction is untouched** — R008 is the same cutover event seen through the relevance gap rather than the concurrency one, and no Change Log activity in either entry can reach a distinction.
  ...
  **The ritual was attempted first and does not terminate.** This bullet and R027's were written, committed, and immediately flagged R007, R008 and R028 — entries already inside the batch. The reference graph has cycles through R028, which nearly every entry cites, so remedy-by-bullet has no fixed point once a batch crosses a commit boundary. That is measured, not argued: the 2026-08-05 bullet below called this pair "a check that manufactures its own next failure", and the manufacture is now shown to be unbounded rather than one-hop.

  The check was therefore widened to walk committed history and date an entry at its last change OUTSIDE the Change Log — the upgrade its own comment named. This bullet stays as the record of what forced it.
...
- 2026-08-05: Cross-reference to R008 re-verified twice, both times because R008 had written a **cross-reference verification bullet of its own** — so both of these bullets said, at length, that a verification bullet elsewhere does not touch this entry's Siblings distinction (the same cutover event seen through the concurrency gap here and the relevance gap there). Consolidated into one, and the underlying defect fixed rather than absorbed: the review-fence check no longer treats a Change-Log-only edit as a move, because its own prescribed remedy is a Change-Log edit, so every remedy re-armed the check one hop further out. This pair is the worked example — a check that manufactures its own next failure.
