...
- 2026-08-10 (third entry today): **Re-verified after R020 was re-scored against the changesets-armed successor route**, which is now BUILT and is the sole path to a production infrastructure apply — `deploy_only` is deleted. **Direction: unchanged, and the earlier note's premise is spent.** That note said every apply this entry describes arrives by the path R020 says has never carried a real mutation — still true, but the path has changed identity: `deploy_only` is deleted and the changesets-armed release-PR merge is the sole route. This entry's subject is what a production `terraform apply` TOUCHES once it starts, not what starts it, so the substitution does not move the score. One genuinely favourable change: every apply is now preceded by a release PR, so there is a place for a plan to be read before the merge — where a dispatch had none. No re-rate.

- 2026-08-10 (second entry today): **Re-verified after [R020](R020-deploy-path-push-tier-prod-deploy-precondition-unmet.active.md)'s treatment and monitoring were re-pointed** off the `deploy_only` dispatch and onto the changeset-armed release-PR route, per the user-ratified JTBD-400 amendment in commit `09f6418`. **Direction for this entry: unchanged, and the earlier entry's closing note is now stale.** That note said every apply this entry describes arrives by the path R020 says has never carried a real mutation — still true, but the path in question changes identity at Phase 4. This entry's subject is what a production `terraform apply` touches once it starts, not what starts it, so a substitution of entry point does not move the score. One second-order observation, favourable and too small to re-rate: an apply arriving on a release-PR merge is preceded by a plan the author reads, whereas a dispatch was not. No re-rate.

- 2026-08-10: **Revisited for the `deploy/**` push-axis retirement.** [R021](R021-push-tier-deploy-axis-arms-prod-terraform-apply.retired.md) and [R022](R022-unstaged-terraform-lockfile-drift-arms-deploy-axis.retired.md) retired (hazard deleted, not reduced); [R020](R020-deploy-path-push-tier-prod-deploy-precondition-unmet.active.md) re-scored 8 → 10 because retiring the axis deleted the ground its Impact 4 rested on. Governance: [ADR 001](../decisions/001-risk-gated-release-process.proposed.md) and [ADR 040](../decisions/040-release-pipeline-change-type-action-matrix.proposed.md), 2026-08-10 amendments. **Direction for this entry: fewer ways to start an apply, same blast radius once one starts.** Its subject is what a production `terraform apply` touches, not what triggers one, so the trigger-count reduction does not move the score. Worth noting the opposite-signed effect rather than only the favourable one: with `deploy_only` now the sole route, every apply this entry describes arrives by the path R020 says has never carried a real mutation. No re-rate; flagged for the next scheduled review.
