...
| Failed-payment access policy                       | SATISFIED | ADR-075, ADR-081 and ADR-082 each record `human-oversight: confirmed` dated 2026-08-28. ADR-076, ADR-079 and ADR-083 were superseded on 2026-09-03 by ADR-087, which matches the shared account's configured recovery. ADR-087 is `human-oversight: unconfirmed` and awaits ratification, so this row is not yet fully re-established. |
...
1. Ratify ADR-087, which records the shared account's recovery policy and
   supersedes ADR-076, ADR-079 and ADR-083. It was captured with derived
   substance and is `human-oversight: unconfirmed`; run
   `/wr-architect:review-decisions` to confirm or amend it.
2. Decide whether the managed channel needs its own alerts before activation.
   The three account policies are email-only and cover none of it, and ADR-081
   expects the paused-collection condition to alert. Building that coverage is
   a change with no documented job behind it yet.
3. Everything policy-gated: activation, any charge, authenticated customer
