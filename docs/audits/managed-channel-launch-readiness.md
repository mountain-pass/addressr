...
2. Archive the legacy AUD metered price on the archived Addressr product. It
   has no subscriptions; the maintainer chose on 2026-09-03 to archive it in
   the Stripe dashboard rather than import it into Terraform, a one-off
   deviation from ADR-085 recorded here because no commercial terms are worth
   managing. An API readback the same day shows the price still active, so the
   archive action is outstanding. It may be the product's default price, which
   Stripe requires be cleared first.
3. Decide whether the managed channel needs its own alerts before activation.
...
4. Everything policy-gated: activation, any charge, authenticated customer
