...
- **As at:** 2026-09-03, after the readbacks and the ADR-087 recovery supersession
...
| Entitlement and failed-payment projection                 | PARTIAL | Behavioural tests cover allowed and denied subscription states, payment-failure non-revocation, paused collection, unsupported payment methods and object-current recovery. Migrated D1 tests additionally prove repeated and concurrent reconciliation repairs policy without resetting same-period usage or crossing ownership boundaries. Stripe Test Clock evidence for recovery, the terminal state ADR-087 now produces (`canceled`, delivered as `customer.subscription.deleted`), restoration and out-of-order delivery is missing.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                     |
...
| Monitoring and alerting                | PARTIAL | Scheduled D1 meter-state checks and the agent reader have run successfully. Workload and provider parity remain unverified. Webhook, entitlement drift, origin and performance alerts, broader quota/accounting coverage and an exercised failure response remain required. Alert-policy readback needs a notifications-read token scope the current token lacks.                                                                                                                                                            |
...
source of truth (RapidAPI maintainer app key, Cloudflare API token, account and
zone IDs, the restricted webhook runtime key and the read-only Stripe key).
...
2. A legacy Addressr product carries an active AUD metered price outside the
   launch catalogue. It has no subscriptions and its product has been archived
   since 2023; the maintainer chose on 2026-09-03 to archive the price in the
   Stripe dashboard rather than import it into Terraform, a one-off deviation
   from ADR-085 recorded here because no commercial terms are worth managing.
   The archive action is still outstanding.
3. Cloudflare token scopes: billing read for the plan readback (ADR-078) and
   notifications read for alert policies, or accept both as UNAVAILABLE.
4. A test-mode Stripe key with write access, held outside production, for the
   Test Clock recovery, restoration and out-of-order scenarios.
5. Everything policy-gated: activation, any charge, authenticated customer
