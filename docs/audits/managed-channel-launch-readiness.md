...
- **As at:** 2026-09-03, after the RapidAPI 4xx counter observation and read-only Cloudflare and Stripe readbacks
...
| Failed-payment access policy                       | SATISFIED | ADR-075, ADR-076, ADR-079, ADR-081, ADR-082 and ADR-083 each record `human-oversight: confirmed` dated 2026-08-28. The 2026-09-03 provider readback contradicts ADR-076, ADR-079 and ADR-083; see the payment-recovery row. |
...
| Stripe payment-recovery settings                          | PARTIAL | DISCREPANCY. Dashboard readback on 2026-09-03 shows card retries enabled with Custom retries at 3, 5 and 7 days and `cancel the subscription` when all retries fail, with the invoice left overdue. That contradicts the Smart Retries, eight-attempt, terminal `unpaid` reading of 2026-08-30 and the drift clauses of ADR-076, ADR-079 and ADR-083. The setting is account-wide and shared with another product, so it was not changed. No automation run or Test Clock recovery has occurred. Activation stays blocked until the maintainer decides which side moves.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                        |
...
1. Failed-payment policy. Stripe applies Custom retries at 3, 5 and 7 days and
   cancels the subscription when they fail; the ratified decisions expect Smart
   Retries and terminal `unpaid`. The setting is shared with another product.
   Either change the account setting, supersede ADR-076, ADR-079 and ADR-083,
   or isolate Addressr billing. Activation stays blocked until decided.
2. A legacy Addressr product carries an active AUD metered price outside the
   launch catalogue; archive it through Terraform or record why it stays.
