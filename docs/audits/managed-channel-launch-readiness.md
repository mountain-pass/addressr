...
| Confidential RapidAPI catalogue parity                    | PARTIAL | ADR-086 limits launch parity to the current public catalogue; historical plans stay on RapidAPI. Authenticated readback covers all four current plan shapes; the user confirmed USD. All four archived Stripe prices match the current base and request charges, including paid allowances and overage. RapidAPI documents one usage increment for origin outcomes below 500, and current Addressr source emits no billing override; the user confirmed monthly reset on each customer subscription anniversary; live counter proof remains unverified. Explicit hard/soft quota policy is deployed. No product was activated or charge authorized.                  |
...
anniversary; authenticated readback established current Stripe price parity. Live counter proof remains unverified. No commercial figures are
stored here.
