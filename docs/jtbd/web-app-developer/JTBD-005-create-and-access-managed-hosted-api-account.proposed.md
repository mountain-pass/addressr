...
screens:
  ...
  - "test/js/__tests__/managed-channel-rehearsal.test.mjs — the browser-origin allowlist exercised through the DEPLOYED SHAPE rather than through the module. ADDED 2026-09-19, inheriting this job's ratification; no outcome changes. The sibling behavioural suite calls the origin filter directly in Node with a synthesised Request; this one builds the Worker bundle through the shipped esbuild invocation and boots it in workerd, so it covers what that suite structurally cannot — that bundling preserves the filter and that the Origin header survives the real request pipeline. THE REFUSED ORIGINS ARE CONFIGURED, NOT OMITTED, and that is the load-bearing part: admission is membership AND shape, so an origin left out of the allowlist is refused for absence and a refusal assertion written that way passes with the shape filter deleted. Placing them inside the allowlist makes the filter the only thing that can refuse them; mutation-proved 2026-09-19 by deleting the filter and watching it red. One case covers an https host merely CONTAINING a loopback literal and records that its 403 is for absence and not shape, because the unchanged https rule admits that host and always did. It carries three annotations, this job's among them."
