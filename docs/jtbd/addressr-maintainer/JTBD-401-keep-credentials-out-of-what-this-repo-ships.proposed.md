...
screens:
  ...
  - "apps/website/test/rendered-output.test.mjs — the build-output scanner. It scans every emitted asset for the Slack webhook host and any `AIza`-shaped Google API key. The old Maps allowlist was retired with the bespoke demo on 2026-08-26, so any Google browser key now fails closed. The file's head-element and autocomplete assertions belong to JTBD-004, not this job."
...
## Known credential in the served bundle — RETIRED 2026-08-26

The bespoke Maps demo and its browser key were removed when the homepage adopted
`@mountainpass/addressr-react`. The build-output guard now asserts that **no** Google API key is emitted,
and the external restriction probe was retired because it no longer has a shipped credential to verify.
The historical investigation below is retained as evidence for why the former allowlist and probe existed.
...
- **Build output is scanned.** **PARTIALLY MET as of 2026-08-23, strengthened 2026-08-26.** `apps/website/test/rendered-output.test.mjs` scans emitted assets for the Slack webhook host and for any `AIza`-shaped key. It still covers one tree and two patterns, not npm tarballs, the Docker image, or arbitrary credential shapes.
...
- **The one credential that legitimately shipped was provably constrained.** **RETIRED 2026-08-26.** The Maps key no longer ships; absence is now asserted from clean Gatsby output, so the former external restriction probe has no subject.

## Reassessment Criteria

- **A Google browser key returns to the website.** The scanner now fails on every `AIza`-shaped key; any deliberate reintroduction must reopen the old restriction and API-surface analysis rather than restoring the former prefix allowlist.
