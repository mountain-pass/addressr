...
screens:
  ...
  - "apps/addressr-deployment/** (Terraform infra source. MOVED TWICE ON 2026-08-10: deploy/** -> packages/deployment/** at commit bf106786, then -> apps/addressr-deployment/** per ADR-046, which splits packages/* (distributable) from apps/* (deployed). AMENDED 2026-08-10: the ADR-040 push axis and its .terraform.lock.hcl exclusion are BOTH retired — a change here no longer triggers anything by path, so there is no pathspec left to exclude from. Worker auth behaviour under apps/addressr-deployment/cloudflare-worker/ was routed from here to JTBD-200, and THAT POINTER WAS WRONG — corrected 2026-09-19 as a factual repoint inheriting this job's ratification, no outcome changed. JTBD-200 does not list that directory at all: its allowlist is the self-hosted operator's Referer check in the website, a different control in a different file for a different persona. The managed gateway's session, role, organisation and browser-origin handling belongs to the customer account job, which names it as of the same date. A pointer naming a job that does not accept the file is worse than no pointer, because it reads as settled and sent three ratified entries into disagreement about one file — this job owns the deploy mechanism only)"
  ...
  - "apps/website/** (the second deployable. AMENDED 2026-08-27 per ADR-060: a consumed `@mountainpass/website` changeset arms the single release workflow's Cloudflare Pages direct upload. Ordinary source pushes do not deploy; the exact merged revision must be served from `addressr.io/revision.txt` before the release is production-verified.)"
  ...
  - 'scripts/detect-deployment-bump.sh (the release.yml deploy-gate detector, landed 8199e5b9 and parameterised 2026-08-27 for ADR-060 — value-not-path comparison of a selected package manifest across the push range, with consumed changesets required. The default remains apps/addressr-deployment/package.json; the website gate passes apps/website/package.json explicitly.)'
