...
screens:
  ...
  - "apps/website/** (the second deployable, landing per ADR-053. IN PHASE 1 IT ARMS NOTHING and that is the entry's whole content: both guards hardcode the deployment path — check-deployment-changeset.sh:73 DEPLOY_DIR and detect-deployment-bump.sh:51 MANIFEST — so a website changeset versions a package and triggers no deploy, and website changes reach production by Netlify push. That is the STATUS QUO carried across the repo move, not a new second pipeline. WHEN PHASE 2 LANDS the two script entries below go false, because they describe both predicates as deployment-tree-scoped and that stops being true the moment a second armed tree exists; ADR-053 carries what phase 2 must do about it. Added 2026-08-23 ahead of the source, deliberately — the drift this list has twice recorded came from adding entries after the code.)"
  - "netlify.toml (repo root) — the phase-1 deploy mechanism for the second deployable. AT THE ROOT DELIBERATELY, and that is why this entry has to exist: base directory must be the repository root because setting it to apps/website breaks npm workspaces (Netlify would install against a directory with no root lockfile), so the file falls OUTSIDE the apps/website/** entry below and is invisible to this job without being named separately. It carries the build command, publish path and an apps/website-scoped ignore predicate. IN PHASE 1 IT ARMS NOTHING: this is the pre-existing unarmed Netlify push path carried across a repository move, changing only which repo Netlify builds from — not a second pipeline against the ONE PIPELINE TO PRODUCTION constraint. ADR-053 pins phase 2 to retire it and to disable the successor's git integration rather than swapping one unarmed path for another. Added 2026-08-23 ahead of the file, deliberately, for the reason the detect-deployment-bump.sh entry below records: this list drifts when entries follow the code."
